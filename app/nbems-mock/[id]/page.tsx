...
export default function NbemsMockRunner() {
  ...
  return (
    <div className="nbm-wrap" style={{ margin: "0 auto", padding: "0 10px 60px", color: "var(--text)" }}>
      {/* Laptop/desktop par poori chaudai: Excel / Word / PPT me kaam wali jagah
          ek taraf badi, instructions doosri taraf. Phone par sab pehle jaisa
          upar-neeche hi rehta hai. */}
      <style>{`
        .nbm-wrap { max-width: 980px; }
        .nbm-type-passage { max-height: 150px; }
        .nbm-type-box { min-height: 300px; }
        @media (min-width: 1100px) {
          .nbm-wrap { max-width: min(1680px, 97vw); }
          .nbm-narrow { max-width: 980px; margin: 0 auto; }
          .nbm-type-box { min-height: max(380px, calc(100vh - 470px)); }
          .nbm-split { display: grid; gap: 14px; align-items: start; }
          .nbm-split.instr-left  { grid-template-columns: minmax(300px, 28%) minmax(0, 1fr); }
          .nbm-split.instr-right { grid-template-columns: minmax(0, 1fr) minmax(300px, 28%); }
          .nbm-split.instr-right .nbm-instr { order: 2; }
          .nbm-split .nbm-instr { position: sticky; top: 150px; max-height: calc(100vh - 160px); overflow-y: auto; }
          .nbm-split .nbm-work { min-width: 0; }
          .nbm-split .wsim-page { min-height: calc(100vh - 290px); }
          .nbm-ppt { max-width: max(560px, calc((100vh - 330px) * 1.7778)); }
          .nbm-sheet table { width: 100%; }
          .nbm-sheet div[tabindex="0"] { max-height: calc(100vh - 290px) !important; }
        }
      `}</style>
      ...
      {/* ════ TYPING ════ */}
      <div className="nbm-narrow" style={{ display: tab === "typing" ? "block" : "none" }}>
        <Card>
      ...
      <div style={{ display: tab === "excel" ? "block" : "none" }}>
        {!data.excel ? (
          ...
          <div className="nbm-split instr-left">
            ...
            <div className="nbm-work nbm-sheet">
      ...
      <div style={{ display: tab === "word" ? "block" : "none" }}>
        {!wordTask ? (
          ...
          <div className="nbm-split instr-right">
            <div className="nbm-instr"><TaskBox title={wordTask.title} intro={wordTask.intro} steps={wordTask.steps} /></div>
            <div className="nbm-work"><WordSimTiptap start={wordStart as WordDoc | undefined} onChange={onWord} /></div>
          </div>
        )}
      </div>
      ...
      <div style={{ display: tab === "ppt" ? "block" : "none" }}>
        {!pptTask ? (
          ...
          <div className="nbm-split instr-right">
            <div className="nbm-instr"><TaskBox title={pptTask.title} intro={pptTask.intro} steps={pptTask.steps} /></div>
            <div className="nbm-work nbm-ppt"><PptSim start={pptStart as PptDoc} onChange={onPpt} /></div>
          </div>
        )}
      </div>
