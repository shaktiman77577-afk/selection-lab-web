    ...
    <div style={{ minHeight: "100vh", background: BG, color: "#fff", display: "flex", flexDirection: "column" }}>
      ...
      >
        <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
          ...
          <button onClick={clearResponse} style={{ ...ghostBtn, flex: 1, fontSize: 12, padding: "10px 6px" }}>
            Clear Response
          </button>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => idx > 0 && setIdx(idx - 1)} disabled={idx === 0} style={{ ...ghostBtn, flex: 1, opacity: idx === 0 ? 0.4 : 1 }}>
            ← Previous
          </button>
          <button onClick={saveAndNext} style={{ ...goldBtn, flex: 2 }}>
            Save & Next
          </button>
        </div>
      </div>
