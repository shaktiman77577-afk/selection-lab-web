...
type Q = {
  id: number;
  image_url?: string;   // DI chart / table / diagram
  question: string;
  ...
  explanation_image_url?: string | null;   // Reasoning ka solution diagram
...
export default function MockTestPlayer() {
  ...
  // Poora analysis — rank, percentile, section-wise (Testbook jaisa)
  const [analysis, setAnalysis] = useState<any>(null);
  const [solIdx, setSolIdx] = useState(0);          // solution view ka apna question index
  const [solSection, setSolSection] = useState(""); // solution view ka section filter
              ...
              if (res && !res.pending) {
                ...
                setResult({
                  ...
                  accuracy: res.attempt?.correct != null && res.attempt?.correct + res.attempt?.wrong > 0
                    ...
                    : 0,
                  pass_percentage: res.test?.pass_percentage,
                  passed: null,
                  reviewing: true,
                });
              }
  ...
  /// Counts — palette ke 5 states ke hisaab se.
  /// `markAns` = mark bhi kiya aur answer bhi diya (asli exam me ye alag ginti hai).
  const stats = useMemo(() => {
    let ans = 0, notAns = 0, mark = 0, markAns = 0, notVisited = 0;
    ...
    // Submit dialog aur result me "kitne attempt kiye" me dono ginte hain
    return { ans, notAns, mark, markAns, notVisited, attempted: ans + markAns };
  }, [questions, answers, marked, visited]);

  /// Palette ke 5 states — bilkul asli exam software jaisa:
  ///   hara         = answer kiya
  ///   purple       = review ke liye mark kiya (answer nahi)
  ///   purple + ✓   = mark bhi kiya AUR answer bhi diya
  ///   laal         = question dekha par answer nahi kiya
  ///   khaali       = abhi khola hi nahi
  ///
  /// Pehle marked sabse upar tha, isliye mark kiya hua question answer karne
  /// par bhi purple hi rehta tha — student ko lagta tha answer save nahi hua.
  function paletteColor(q: Q) {
    const k = String(q.id);
    const isAns = !!answers[k];
    const isMark = !!marked[k];

    if (isMark && isAns) return { bg: PURPLE, fg: "#fff", tick: true };
    if (isMark) return { bg: PURPLE, fg: "#fff", tick: false };
    if (isAns) return { bg: GREEN, fg: "#fff", tick: false };
    if (visited[k]) return { bg: RED, fg: "#fff", tick: false };
    return { bg: "var(--chip)", fg: "var(--text)", tick: false };
  }
  ...
  // Rank, percentile aur section-wise breakdown
  async function loadAnalysis() {
    try {
      const u = getUser();
      const r = await fetch(`${API_URL}/mock-tests/${testId}/result?user_id=${(u as any)?.id}`);
      const d = await r.json();
      if (r.ok && !d.pending) setAnalysis(d);
    } catch {}
  }
  ...
  async function openSolutions() {
    ...
    setSolIdx(0);
    setSolSection(questions[0]?.section || "");
  ...
  // ── SOLUTIONS — apna poora page, Testbook jaisa ──
  if (result && showSolutions) {
    const solQ = questions[solIdx];
    const sol = solQ ? solutions[String(solQ.id)] || {} : {};
    const correct = sol.correct_answer ?? (solQ as any)?.correct_answer;
    const given = solQ ? answers[String(solQ.id)] : undefined;
    const ok = given === correct;

    // Counts
    let nCorrect = 0, nWrong = 0, nSkip = 0;
    for (const q of questions) {
      const c = solutions[String(q.id)]?.correct_answer ?? (q as any).correct_answer;
      const g = answers[String(q.id)];
      if (!g) nSkip++;
      else if (g === c) nCorrect++;
      else nWrong++;
    }
    ...
    const statusOf = (q: Q) => {
      const c = solutions[String(q.id)]?.correct_answer ?? (q as any).correct_answer;
      const g = answers[String(q.id)];
      return !g ? "skipped" : g === c ? "correct" : "wrong";
    };
    const colorOf = (st: string) => (st === "correct" ? GREEN : st === "wrong" ? RED : "var(--chip)");

    const visible = questions
      .map((q, i) => ({ q, i }))
      .filter(({ q }) => (solFilter === "all" ? true : statusOf(q) === solFilter));

    const qText = solQ ? (lang === "hi" && solQ.question_hi ? solQ.question_hi : solQ.question) : "";
    const exp = lang === "hi" && (sol.explanation_hi ?? solQ?.explanation_hi)
      ? (sol.explanation_hi ?? solQ?.explanation_hi)
      : (sol.explanation ?? solQ?.explanation);

    // Diagram /solutions se aata hai (exam=1 par question payload me nahi hota)
    const expImg = (sol as any).explanation_image_url ?? solQ?.explanation_image_url ?? null;

    const solPalette = (
      <>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12 }}>
          {([
            ["all", `All ${questions.length}`, "var(--text)"],
            ["correct", `${nCorrect} Correct`, GREEN],
            ["wrong", `${nWrong} Incorrect`, RED],
            ["skipped", `${nSkip} Unattempted`, "var(--muted)"],
          ] as const).map(([key, label, col]) => (
            <button
              key={key}
              onClick={() => setSolFilter(key as any)}
              style={{
                fontSize: 11, fontWeight: 700, padding: "5px 10px", borderRadius: 8, cursor: "pointer",
                border: `1px solid ${solFilter === key ? col : "var(--line)"}`,
                background: solFilter === key ? `${col}22` : "transparent",
                color: solFilter === key ? col : "var(--muted)",
              }}
            >
              {label}
            </button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(42px, 1fr))", gap: 8 }}>
          {visible.map(({ q, i }) => {
            ...
            return (
              <button
                key={q.id}
                onClick={() => { setSolIdx(i); setShowPalette(false); }}
                style={{
                  height: 42, borderRadius: 10, fontWeight: 800, fontSize: 13, cursor: "pointer",
                  background: st === "skipped" ? "var(--chip)" : c,
                  color: st === "skipped" ? "var(--text)" : "#fff",
                  border: i === solIdx ? `2px solid ${GOLD}` : "1px solid var(--line)",
                }}
              >
                {i + 1}
              </button>
    ...
    return (
      <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", paddingBottom: 110 }}>
        {/* Header */}
        <div
          style={{
            position: "sticky", top: 0, zIndex: 20, background: "var(--header)",
            borderBottom: "1px solid var(--line)", padding: wide ? "10px 18px" : "8px 12px",
            display: "flex", alignItems: "center", gap: 10,
          }}
        >
          <button onClick={() => setShowSolutions(false)} style={{ ...ghostBtn, padding: "7px 12px" }}>←</button>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 800, fontSize: wide ? 15 : 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {test?.title}
            </div>
            <div style={{ fontSize: 11, color: "var(--muted)" }}>Solutions</div>
          </div>
          <button
        ...
        {/* Mobile counts strip */}
        {!wide && (
          <div style={{ display: "flex", gap: 7, overflowX: "auto", padding: "7px 12px", borderBottom: "1px solid var(--line)", background: "var(--card)", fontSize: 10.5 }}>
            <Pill n={nCorrect} label="Correct" color={GREEN} />
            <Pill n={nWrong} label="Incorrect" color={RED} />
            <Pill n={nSkip} label="Unattempted" color="var(--muted)" />
          </div>
        )}
            ...
            {sections.map((s) => (
              <button
                ...
                onClick={() => {
                  ...
                  if (first >= 0) setSolIdx(first);
                  setSolSection(s);
                }}
                style={{
                  flexShrink: 0, padding: "6px 12px", borderRadius: 20, fontSize: 12.5, fontWeight: 700, cursor: "pointer",
                  border: `1px solid ${solQ?.section === s ? GOLD : "var(--line)"}`,
                  background: solQ?.section === s ? "rgba(255,171,0,0.14)" : "var(--card)",
                  color: solQ?.section === s ? GOLD : "var(--text)",
                }}
              >
                {s}
              </button>
        ...
        <div style={{ display: "flex", gap: 16, maxWidth: wide ? 1120 : 640, margin: "0 auto", padding: 14, alignItems: "flex-start" }}>
          {/* Question + solution */}
          <div style={{ flex: 1, minWidth: 0, background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: wide ? 22 : 15 }}>
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", paddingBottom: 11, marginBottom: 13, borderBottom: "1px solid var(--line)" }}>
              <b style={{ fontSize: 13.5 }}>Question {solIdx + 1}</b>
              <span
                style={{
                  fontSize: 10.5, fontWeight: 800, padding: "3px 9px", borderRadius: 6,
                  color: !given ? "var(--muted)" : ok ? GREEN : RED,
                  background: !given ? "var(--chip)" : ok ? "rgba(46,139,74,0.14)" : "rgba(214,69,69,0.14)",
                }}
              >
                {!given ? "SKIPPED" : ok ? "CORRECT" : "INCORRECT"}
              </span>
              ...
              {sol.my_seconds != null && (
                <span style={{ fontSize: 11, color: "var(--muted)" }}>
                  ...
                  {sol.avg_seconds != null && (
                    <b style={{ marginLeft: 6, color: sol.my_seconds <= sol.avg_seconds * 0.7 ? GREEN : sol.my_seconds <= sol.avg_seconds * 1.3 ? GOLD : RED }}>
                      {sol.my_seconds <= sol.avg_seconds * 0.7 ? "⚡ Fast" : sol.my_seconds <= sol.avg_seconds * 1.3 ? "On time" : "🐢 Slow"}
                    </b>
                  )}
                </span>
              )}
              ...
              <span style={{ marginLeft: "auto", fontSize: 11.5, color: "var(--muted)" }}>
                Marks: <b style={{ color: !given ? "var(--muted)" : ok ? GREEN : RED }}>
                  {!given ? 0 : ok ? `+${solQ?.marks}` : `−${test?.negative_marking || 0}`}
                </b>
              </span>
            </div>

            {sol.correct_pct != null && (
              <div style={{ display: "inline-block", fontSize: 11, fontWeight: 700, color: GREEN, background: "rgba(46,139,74,0.12)", padding: "3px 10px", borderRadius: 6, marginBottom: 11 }}>
            ...
            {solQ?.image_url ? (
              <div
                onClick={() => setZoomImg(solQ.image_url!)}
                style={{
                  marginBottom: 13, borderRadius: 12, overflow: "hidden",
                  border: "1px solid var(--line)", background: "#fff", cursor: "zoom-in",
                }}
              >
                <img src={solQ.image_url} alt="Question chart" style={{ width: "100%", display: "block" }} />
              </div>
            ) : null}

            <RichText text={qText} style={{ display: "block", fontSize: wide ? 16.5 : 15, lineHeight: 1.75 }} />
            ...
            <div style={{ marginTop: 14 }}>
              {LETTERS.map((L) => {
                ...
                const txt = (lang === "hi" && solQ[keyHi] ? solQ[keyHi] : solQ[key]) as string;
                if (!txt) return null;
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${isCorrect ? GREEN : isGiven ? RED : "var(--line)"}`,
                      background: isCorrect ? "rgba(46,139,74,0.12)" : isGiven ? "rgba(214,69,69,0.10)" : "transparent",
                    }}
                  >
                    <span style={{ fontWeight: 800, fontSize: 13.5, color: isCorrect ? GREEN : isGiven ? RED : "var(--muted)", flexShrink: 0 }}>
                      {isCorrect ? "✓" : isGiven ? "✗" : L}
                    </span>
                    <RichText text={txt} style={{ fontSize: 14.5, lineHeight: 1.6, flex: 1 }} />
                    {isGiven && !isCorrect && (
                      <span style={{ fontSize: 10, fontWeight: 800, color: RED, flexShrink: 0 }}>YOUR ANSWER</span>
                    )}
                  </div>
                );
              })}
            ...
            {exp && (
              <div style={{ marginTop: 14, background: "var(--chip)", borderRadius: 11, padding: 14 }}>
                <div style={{ fontSize: 11.5, fontWeight: 800, color: GOLD, letterSpacing: 0.6, marginBottom: 6 }}>
                  EXPLANATION
                </div>
                <RichText text={exp} style={{ display: "block", fontSize: 14, lineHeight: 1.75 }} />

                {/* Reasoning ka diagram — seating arrangement, family tree, puzzle grid.
                    Tap karke bada karke dekh sakte hain. */}
                {expImg ? (
                  <div
                    onClick={() => setZoomImg(expImg)}
                    style={{
                      marginTop: 12, borderRadius: 10, overflow: "hidden",
                      border: "1px solid var(--line)", background: "#fff", cursor: "zoom-in",
                    }}
                  >
                    <img src={expImg} alt="Solution diagram" style={{ width: "100%", display: "block" }} />
                    <div style={{ fontSize: 10.5, color: "#555", textAlign: "center", padding: "4px 0", background: "#f4f4f4" }}>
                      Tap to zoom
                    </div>
                  </div>
                ) : null}
          ...
          {/* Desktop palette */}
          {wide && (
            <aside style={{ width: 300, flexShrink: 0, position: "sticky", top: 86, background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: 16, maxHeight: "calc(100vh - 130px)", overflowY: "auto" }}>
        ...
        {/* Bottom nav */}
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 20, background: "var(--header)", borderTop: "1px solid var(--line)", padding: "10px 12px" }}>
          <div style={{ display: "flex", gap: 8, maxWidth: wide ? 1120 : 640, margin: "0 auto" }}>
            <button onClick={() => setSolIdx((i) => Math.max(0, i - 1))} disabled={solIdx === 0} style={{ ...ghostBtn, flex: 1, padding: "12px 0", opacity: solIdx === 0 ? 0.45 : 1 }}>
              ← Previous
            </button>
            {!wide && (
              <button onClick={() => setShowPalette(true)} style={{ ...ghostBtn, flex: 1, padding: "12px 0", fontSize: 12.5 }}>
                ☰ {solIdx + 1}/{questions.length}
              </button>
            )}
            <button onClick={() => setSolIdx((i) => Math.min(questions.length - 1, i + 1))} disabled={solIdx === questions.length - 1} style={{ ...goldBtn, flex: 1.4, padding: "12px 0", opacity: solIdx === questions.length - 1 ? 0.45 : 1 }}>
              Next →
            </button>
          </div>
        </div>

        {/* Chart zoom — solutions me bhi chahiye */}
        {zoomImg && (
          <div
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                color: "#fff", border: "1px solid rgba(255,255,255,0.35)", borderRadius: 10,
        ...
        {showPalette && !wide && (
          <div onClick={() => setShowPalette(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 30, display: "flex", justifyContent: "flex-end" }}>
            <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--bg)", width: "min(88vw, 340px)", height: "100%", overflowY: "auto", padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <b style={{ fontSize: 16 }}>Questions</b>
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", padding: 16 }}>
          ...
          >
            ...
            <div style={{ fontSize: 13, opacity: 0.85, marginTop: 4 }}>
              {result.reviewing ? "Your earlier attempt" : result.auto ? "Time up — auto submitted" : "Test submitted"}
            </div>
            ...
            {result.passed != null && (
              <div style={{ fontSize: 14, marginTop: 6, fontWeight: 700, color: result.passed ? "#5dd97c" : "#ffb4b4" }}>
                {result.passed
                  ? "PASSED ✓"
                  : result.pass_percentage != null
                    ? `Not cleared — you needed ${result.pass_percentage}%`
                    : "Not cleared"}
              </div>
            )}
          </div>

          {/* Overall Performance Summary — Testbook jaisa */}
          <h3 style={{ fontSize: 15.5, fontWeight: 800, margin: "18px 0 10px" }}>Overall Performance</h3>
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: 16 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 14 }}>
              {analysis?.rank != null && (
                <Metric icon="🏅" bg="#d6285a" value={`${analysis.rank}`} sub={`/ ${analysis.total_attempts}`} label="Rank" />
              )}
              <Metric icon="🏆" bg="#7c4dff" value={`${result.score}`} sub={`/ ${result.total_marks}`} label="Score" />
              <Metric
                icon="📝" bg="#22a9d6"
                value={`${(result.correct || 0) + (result.wrong || 0)}`}
                sub={`/ ${(result.correct || 0) + (result.wrong || 0) + (result.skipped || 0)}`}
                label="Attempted"
              />
              <Metric icon="🎯" bg="#2fa98c" value={`${result.accuracy ?? 0}%`} label="Accuracy" />
              {analysis?.percentile != null && (
                <Metric icon="👥" bg="#6b5ce7" value={`${analysis.percentile}%`} label="Percentile" />
              )}
            </div>
          </div>
          ...
          {/* Sectional Summary — table, Testbook jaisa */}
          {analysis?.sections?.length > 0 && (
            <>
              <h3 style={{ fontSize: 15.5, fontWeight: 800, margin: "18px 0 10px" }}>Sectional Summary</h3>
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, overflow: "hidden" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 0.9fr", gap: 6, padding: "10px 13px", background: "var(--chip)", fontSize: 11, fontWeight: 800, color: "var(--muted)" }}>
                  <span>SECTION</span>
                ...
                {analysis.sections.map((s: any) => (
                  <div
                    key={s.section}
                    style={{
                      display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 0.9fr", gap: 6,
                      padding: "12px 13px", borderTop: "1px solid var(--line)", fontSize: 13, alignItems: "center",
                    }}
                  >
                    <span style={{ fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis" }}>{s.section}</span>
                    <span style={{ textAlign: "right", fontWeight: 800 }}>
                      {s.score}<span style={{ color: "var(--muted)", fontWeight: 600, fontSize: 11.5 }}> / {s.max_marks}</span>
                    </span>
                    <span style={{ textAlign: "right", color: "var(--muted)", fontSize: 12.5 }}>
                      {s.attempted} / {s.total}
                    </span>
                    <span style={{ textAlign: "right", fontWeight: 800, color: s.accuracy >= 75 ? GREEN : s.accuracy >= 50 ? GOLD : RED }}>
                      {s.accuracy}%
                    </span>
                  </div>
                ))}
                <div
                  style={{
                    display: "grid", gridTemplateColumns: "1.6fr 1fr 1fr 0.9fr", gap: 6,
                    padding: "12px 13px", borderTop: `2px solid ${GOLD}44`, fontSize: 13.5, fontWeight: 800, alignItems: "center",
                  }}
                >
                  <span>Overall</span>
                  <span style={{ textAlign: "right", color: GOLD }}>
                    {result.score}<span style={{ color: "var(--muted)", fontWeight: 600, fontSize: 11.5 }}> / {result.total_marks}</span>
                  </span>
                  <span style={{ textAlign: "right", color: "var(--muted)", fontSize: 12.5, fontWeight: 600 }}>
                    {(result.correct || 0) + (result.wrong || 0)} / {questions.length}
                  </span>
                  <span style={{ textAlign: "right", color: GOLD }}>{result.accuracy ?? 0}%</span>
                </div>
              </div>
            </>
          )}

          {/* Weakness aur Strengths */}
          {(analysis?.weak_areas?.length > 0 || analysis?.strong_areas?.length > 0) && (
            <>
              <h3 style={{ fontSize: 15.5, fontWeight: 800, margin: "18px 0 10px" }}>Your Weakness and Strengths</h3>
              {analysis.strong_areas?.length > 0 && (
                <div style={{ background: "rgba(46,139,74,0.10)", border: `1px solid ${GREEN}55`, borderRadius: 14, padding: "13px 15px", marginBottom: 10 }}>
                  <div style={{ fontSize: 11.5, fontWeight: 800, color: GREEN, letterSpacing: 0.6 }}>💪 STRONG</div>
                  <div style={{ fontSize: 14, marginTop: 4 }}>{analysis.strong_areas.join(" · ")}</div>
                </div>
              )}
              {analysis.weak_areas?.length > 0 && (
                <div style={{ background: "rgba(214,69,69,0.10)", border: `1px solid ${RED}55`, borderRadius: 14, padding: "13px 15px" }}>
                  <div style={{ fontSize: 11.5, fontWeight: 800, color: RED, letterSpacing: 0.6 }}>📌 NEEDS WORK</div>
                  <div style={{ fontSize: 14, marginTop: 4 }}>{analysis.weak_areas.join(" · ")}</div>
          ...
          {/* Comparison — topper aur average */}
          {analysis?.topper_score != null && (
            <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: 16, marginTop: 14 }}>
              <div style={{ fontWeight: 800, fontSize: 14.5, marginBottom: 10 }}>How you compare</div>
              <CompareRow label="Topper's score" value={`${analysis.topper_score} / ${result.total_marks}`} />
              <CompareRow label="Average score" value={`${analysis.average_score} / ${result.total_marks}`} />
              <CompareRow label="Your score" value={`${result.score} / ${result.total_marks}`} strong />
              <CompareRow label="Students attempted" value={String(analysis.total_attempts)} />
            </div>
          )}
  ...
  const paletteBody = (
    <>
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", fontSize: 11.5, color: "var(--muted)", marginBottom: 12 }}>
        <Legend color={GREEN} text={`Answered (${stats.attempted})`} />
        <Legend color={RED} text={`Not answered (${stats.notAns})`} />
        <Legend color={PURPLE} text={`Marked (${stats.mark})`} />
        <Legend color={PURPLE} tick text={`Marked & answered (${stats.markAns})`} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(42px, 1fr))", gap: 8 }}>
        {questions.map((q, i) => {
          ...
          return (
            ...
            >
              {i + 1}
              {/* Mark kiya hua question answer bhi ho gaya — hara tick */}
              {c.tick && (
                <span
                  style={{
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", paddingBottom: 150 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!wide && (
            <div style={{ fontSize: 10.5, color: "var(--muted)" }}>
              Q {idx + 1}/{questions.length} · {stats.attempted} answered
            </div>
          )}
        </div>
        <div
      ...
      {!wide && (
        ...
        >
          <Pill n={stats.attempted} label="Answered" color={GREEN} />
      ...
      <div style={{ display: "flex", gap: 16, maxWidth: wide ? 1120 : 640, margin: "0 auto", padding: 14, alignItems: "flex-start" }}>
        ...
        >
          ...
          {/* DI chart / table — question se pehle, taaki padhne se pehle dikh jaye.
              Tap karke bada karke dekh sakte hain (chhote number sahi se padhne ke liye). */}
          {current.image_url ? (
            <div
              onClick={() => setZoomImg(current.image_url!)}
              style={{
          ...
          <RichText text={qText} style={{ display: "block", fontSize: wide ? 17 : 15, lineHeight: 1.75 }} />
      ...
      >
        ...
        {!wide && (
          <button onClick={() => setShowPalette(true)} style={{ ...ghostBtn, width: "100%", marginTop: 8, fontSize: 12.5, padding: "8px 0" }}>
            ☰ Question Palette ({stats.attempted} answered · {questions.length - stats.attempted} left)
          </button>
        )}
      </div>
            ...
            {questions.map((q, i) => {
              ...
              return (
                ...
                >
                  ...
                  <RichText text={qt} style={{ display: "block", fontSize: 13.5, lineHeight: 1.55 }} />
                  {LETTERS.map((L) => {
                    const key = `option_${L.toLowerCase()}` as keyof Q;
                    const keyHi = `option_${L.toLowerCase()}_hi` as keyof Q;
      ...
      {/* Chart zoom — DI ke chhote numbers padhne ke liye */}
      {zoomImg && (
        <div
          onClick={() => setZoomImg(null)}
          style={{
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 40, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 16, padding: 20, maxWidth: 380, width: "100%" }}>
            ...
            <div style={{ fontSize: 13.5, color: "var(--muted)", margin: "10px 0 14px", lineHeight: 1.6 }}>
              {/* Marked-but-unanswered ko "answered" me nahi ginte — pehle
                  ginte the, isliye student ko lagta tha sab ho gaya. */}
              Answered: <b style={{ color: GREEN }}>{stats.attempted}</b> · Left:{" "}
              <b style={{ color: RED }}>{questions.length - stats.attempted}</b>
              {stats.mark > 0 && (
                <>
                  <br />
                  <span style={{ color: PURPLE, fontWeight: 700 }}>
                    {stats.mark} marked for review {stats.mark === 1 ? "is" : "are"} still unanswered.
                  </span>
                </>
              )}
...
function Metric({ icon, bg, value, sub, label }: { icon: string; bg: string; value: string; sub?: string; label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ fontSize: 18, fontWeight: 800, lineHeight: 1.2 }}>
          {value}
          {sub && <span style={{ fontSize: 12.5, color: "var(--muted)", fontWeight: 600 }}> {sub}</span>}
        </div>
...
function CompareRow({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13.5, padding: "8px 0", borderTop: "1px solid var(--line)" }}>
...
function Legend({ color, text, tick }: { color: string; text: string; tick?: boolean }) {
  return (
    <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
      <span
        style={{
          position: "relative", width: 12, height: 12, borderRadius: 3,
          background: color, border: "1px solid var(--line)",
        }}
      >
        {tick && (
          <span
            style={{
              ...
              background: "#2e8b4a", border: "1.5px solid var(--card)",
            }}
          />
        )}
      </span>
