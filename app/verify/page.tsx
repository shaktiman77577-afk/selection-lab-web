  ...
  return (
    <>
      ...
      <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderTop: "none", borderRadius: "0 0 12px 12px", padding: 14 }}>
        ...
        <div style={{ fontSize: 14.5, lineHeight: 1.65, fontWeight: 600, marginBottom: 14 }}>
          {(hi && q.question_hi) || q.question}
        </div>
      ...
      {shown.map((q: any) => {
        ...
        return (
          ...
          >
            ...
            <div style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.6 }}>{q.question}</div>
            {showHi && q.question_hi && (
              <div style={{ fontSize: 13.5, color: "var(--text2)", lineHeight: 1.65, marginTop: 5 }}>{q.question_hi}</div>
            )}
            {showHi && !q.question_hi && (
              ...
              {LETTERS.map((L) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <div>{en}</div>
                      {showHi && hiOpt && <div style={{ color: "var(--muted)", fontSize: 12.5, marginTop: 2 }}>{hiOpt}</div>}
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ background: "rgba(255,171,0,0.07)", border: "1px solid rgba(255,171,0,0.25)", borderRadius: 9, padding: 10, marginTop: 9 }}>
              ...
              {q.explanation ? (
                <div style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.65 }}>{q.explanation}</div>
              ) : (
                <div style={{ fontSize: 12.5, color: RED, fontWeight: 700 }}>⚠️ Explanation nahi hai</div>
              )}
              {showHi && q.explanation_hi && (
                <div style={{ fontSize: 12.5, color: "var(--muted)", lineHeight: 1.65, marginTop: 6 }}>{q.explanation_hi}</div>
              )}
            </div>
