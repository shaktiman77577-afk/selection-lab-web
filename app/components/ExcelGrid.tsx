    ...
    <div>
      <div
        ...
        style={{
          ...
          // Formula type karte waqt ungli se range chunni hai (C2 se C7 —
          // upar-neeche). "pan-y" par browser upar-neeche kheenchne ko page
          // scroll maan kar drag cancel kar deta tha, aur sirf pehla cell
          // (C2) judta tha. Students ki "drag nahi hota" wali shikayat yahi
          // thi. Type karte waqt grid par browser ka scroll band; baaki
          // waqt pehle jaisa, taaki grid par ungli rakh kar page scroll ho.
          touchAction: pickMode ? "none" : "pan-y",
                ...
                {Array.from({ length: cols }).map((_, c) => {
                  ...
                  return (
                    ...
                    >
                      {text}
                      {showHandle && (
                        // Dikhne me wahi chhota chaukor, par ungli ke liye
                        // pakadne ki jagah 26px. 9px ko phone par pakadna
                        // lagbhag namumkin tha. touchAction "none": warna
                        // neeche kheenchte hi browser page scroll karne lagta
                        // aur fill beech me hi (D2:D3 par) ruk jata.
                        <span
                          onPointerDown={(e) => { e.stopPropagation(); startDrag("fill", ref, e); }}
                          title="Drag down to fill"
                          style={{
                            position: "absolute", right: -5, bottom: -5,
                            width: 26, height: 26, zIndex: 3,
                            // Chaukor kone par, pakadne ki jagah zyadatar cell ke
                            // andar — aakhri column me grid ka kinara ise kaate nahi
                            display: "flex", alignItems: "flex-end", justifyContent: "flex-end",
                            cursor: "crosshair", touchAction: "none",
                          }}
                        >
                          <span style={{ width: 9, height: 9, background: tone, border: "1px solid #fff" }} />
                        </span>
                      )}
                    </td>
                  );
...
export function FormulaBar({
  ...
  // Lamba formula (IFERROR(VLOOKUP(...))) phone par ek line me nahi samata —
  // shuru ka hissa chhup jata tha aur student ko lagta tha "poora type nahi
  // ho raha". Ab box neeche badhta hai aur poora formula dikhta hai.
  useEffect(() => {
    ...
    <div style={{ position: "relative", marginTop: 10 }}>
      <div style={{ display: "flex", alignItems: "stretch", border: `1px solid ${LINE}`, borderRadius: 6, background: "#fff", overflow: "hidden" }}>
        ...
        <textarea
          ...
          onKeyUp={track}
          onKeyDown={(e) => {
            // Enter = jaanch, nayi line nahi (formula ek hi line ka hota hai)
            if (e.key === "Enter") { e.preventDefault(); setOpen(false); onSubmit?.(); }
            if (e.key === "Escape") setOpen(false);
          }}
          // Pehle "=SUM(A1:A5)" likha rehta tha — student samajhte the ki
          // formula pehle se bhara hai, bas range badalni hai.
          placeholder={placeholder || "Type = and then your formula"}
          spellCheck={false}
          autoCorrect="off"
          // Pehle "characters" tha — phone ka keyboard SAB KUCH capital me
          // likhta tha, quotes ke andar ka text bhi. "Not found!" apne aap
          // "NOT FOUND" ban jata aur jawab galat maana jata (text me case
          // maayne rakhta hai). Function ke naam chhote me bhi chalte hain —
          // engine khud bade kar leta hai.
          autoCapitalize="none"
          autoComplete="off"
          enterKeyHint="go"
          style={{
