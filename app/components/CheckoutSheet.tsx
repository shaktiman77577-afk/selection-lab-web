...
const LABELS: Record<string, { tag: string; emoji: string }> = {
  ...
  tier2: { tag: "TIER 2 PRACTICE", emoji: "⌨️" },
};
...
export default function CheckoutSheet({
  ...
  open: boolean;
  onClose: () => void;
  productType: "course" | "mock" | "descriptive" | "tier2";
    ...
    try {
      const res = await fetch(`${API_URL}/coupons/validate`, {
        ...
        body: JSON.stringify({
          ...
          // "Aap ye coupon pehle use kar chuke hain" yahin pata chal jaye
          user_id: (getUser() as any)?.id ?? null,
        }),
      });
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "var(--bg)", zIndex: 25, overflowY: "auto" }}>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "12px 16px", background: "var(--header)", borderTop: "1px solid var(--line)", zIndex: 26 }}>
        ...
        >
          {paying ? "Please wait..." : applied && total <= 0 ? "🎉 Unlock free with coupon" : `🔒 Pay Securely  ·  ₹${total}`}
        </button>
      </div>
    </div>
  );
