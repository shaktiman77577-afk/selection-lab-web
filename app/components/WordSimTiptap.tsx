...
export default function WordSimTiptap({ start, onChange }: { start?: WordDoc; onChange: (d: WordDoc) => void }) {
  // Mock me resume par pehle se "Save As" kiya naam bhi wapas aaye. Practice
  // tasks ka start hamesha khaali naam deta hai, isliye unka kuch nahi badalta.
  const [fileName, setFileName] = useState(start?.fileName || "");
  const nameRef = useRef(start?.fileName || "");
  ...
  if (!editorOrNull) return <div style={{ padding: 20, color: "#666" }}>Loading the editor…</div>;
  // Check ke baad pakka naam — neeche ke function bhi jaanein ki editor khaali nahi
  // (TypeScript function declarations me upar wala check nahi dekhta)
  const editor: Editor = editorOrNull;
  ...
  function withParagraph(fn: (c: ReturnType<typeof ch>) => any) {
