...
export function SheetEditor({
  ...
  function undo() {
    ...
    if (prev) { setEditing(null); setDraft(""); onChange(prev); }
  }
                  ...
                  return (
                    <td
                      ...
                      onPointerUp={() => {
                        ...
                        drag.current = null;
                        if (!d) return;
                        if (d.mode === "fill") { if (d.moved) fillTo({ r, c }); setFillTo(null); return; }
                        ...
                        // hai. Par ungli kheenchi ho (range chunna) to nahi
                        // — warna bold/merge ke liye cell chunna hi mushkil
                        // ho jata.
                        if (!d.moved && isTouch.current && d.from.r === r && d.from.c === c) {
                          startEdit(r, c);
                        }
                      }}
                      ...
                      {!disabled && !isEdit && r === sel.r2 && c === sel.c2 && (
                        <span
                          ...
                          style={{
                            ...
                            width: 11, height: 11, background: BLUE,
                            border: "2px solid #fff", borderRadius: 2,
                            cursor: "crosshair", zIndex: 4, touchAction: "none",
                          }}
                        />
                      )}
                      {isEdit ? (
