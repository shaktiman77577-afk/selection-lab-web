...
export default function WorksheetTest() {
  ...
  const submitXlsx = useCallback(async (f: File) => {
    ...
    try {
      ...
      const r = await fetch(`${API_URL}/tier2/excel/worksheet/submit-xlsx`, { method: "POST", body: fd });
      const d = await r.json();
      if (!r.ok) throw new Error(d.detail || "File jaanchi nahi ja saki");
      setResult(d); setStage("result");
    ...
    return (
      <Shell>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)",
          ...
          {([
            ["web", "Isi website par", "Phone ya laptop — sheet yahin bhariye"],
            ["xlsx", "Apne laptop ke Excel me", "Excel / LibreOffice / WPS — .xlsx file yahan upload kijiye"],
          ...
          {mode === "xlsx" && (
            <p style={{ fontSize: 11.5, color: "var(--muted)", lineHeight: 1.65, margin: "2px 0 0" }}>
              Kaam ka waqt utna hi — {test?.duration_min} minute. Uske baad file save
              karke upload karne ke liye <b style={{ color: "var(--text)" }}>2 minute</b> aur
              milenge. File <b style={{ color: "var(--text)" }}>.xlsx</b> me save kijiye.
            </p>
          )}
        </div>
      ...
      <Shell>
        ...
        {mode === "web" ? (
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--line)",
            ...
            <b>Apne Excel me banaiye</b>
            <div style={{ color: "var(--muted)", marginTop: 6 }}>
              Kagaz saamne rakhiye aur worksheet apne laptop par bana lijiye.
              Table <b style={{ color: "var(--text)" }}>{test?.start_cell}</b> se shuru
              kijiye. Khaali column me formula lagaiye — haath se jawab likhne par
              marks nahi milte.<br />
              Kaam poora hote hi file ko <b style={{ color: "var(--text)" }}>.xlsx</b> me
              save kijiye. Timer khatam hote hi upload ka page khul jayega.
            </div>
          </div>
        )}
        ...
        <button onClick={() => (mode === "xlsx" ? setStage("upload") : submit())}
          ...
          {busy ? "Submitting…" : mode === "xlsx" ? "Ho gaya — file upload karni hai" : "Submit"}
        </button>
    ...
    return (
      <Shell>
        <div style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--card)",
                      border: "1px solid var(--line)", borderRadius: 10, padding: "9px 12px",
                      marginBottom: 12 }}>
          <span style={{ fontSize: 12.5, fontWeight: 800 }}>File upload kijiye</span>
          <span style={{ flex: 1 }} />
          <span style={{ fontSize: 14, fontWeight: 800, fontVariantNumeric: "tabular-nums",
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)",
          ...
          <b>Apni .xlsx file chuniye</b>
          <div style={{ color: "var(--muted)", marginTop: 6 }}>
            Excel me <b style={{ color: "var(--text)" }}>Save As → Excel Workbook (.xlsx)</b>.
            LibreOffice ya WPS me bhi ".xlsx" chuniye — teeno ek hi file banate hain.
            Jaanch wahi hai jo website wali sheet ki hoti hai.
          </div>
        </div>
        ...
        {late && (
          <p style={{ fontSize: 12.5, color: RED, lineHeight: 1.7, marginBottom: 12 }}>
            Upload ka waqt nikal gaya. File ab bhi jaanchi jayegi, par asli exam me
            der se diya gaya printout nahi liya jata.
          </p>
        )}
        ...
        <button onClick={() => file.current?.click()} disabled={busy} style={gold}>
          {busy ? "Jaanchi ja rahi hai…" : "📄 File chuniye aur submit kijiye"}
        </button>
        ...
        <p style={{ fontSize: 12, color: "var(--muted)", lineHeight: 1.7, marginTop: 14 }}>
          File nahi mil rahi? Excel me File → Save As dekhiye ki kis folder me gayi hai.
        </p>
      </Shell>
    );
  }
    ...
    <Shell>
      ...
      {result.layout_notes?.length > 0 && (
        <div style={{ background: "rgba(255,171,0,0.08)", border: `1px solid ${GOLD}`,
                      borderRadius: 12, padding: 14, marginBottom: 14, fontSize: 13.5, lineHeight: 1.75 }}>
          <b style={{ color: GOLD }}>Table ka dhancha thoda alag tha</b>
          <div style={{ color: "var(--muted)", marginTop: 6 }}>
            {result.layout_notes.map((d: any, i: number) => (
              <div key={i}>
                {d.op === "blank_row_missing"
                  ? <>Page par <b style={{ color: "var(--text)" }}>row {d.ref}</b> khaali chhodi gayi hai — aapne wahan likh diya.</>
                  : d.op === "blank_row_extra"
                  ? <>Aapne <b style={{ color: "var(--text)" }}>row {d.ref}</b> khaali chhodi, page par wo khaali nahi hai.</>
                  : d.op === "blank_col_missing"
                  ? <>Page par <b style={{ color: "var(--text)" }}>column {d.ref}</b> khaali hai — aapne wahan likh diya.</>
                  : <>Aapne <b style={{ color: "var(--text)" }}>column {d.ref}</b> khaali chhoda, page par wo khaali nahi hai.</>}
              </div>
            ))}
            <span style={{ color: GREEN }}>
              Baaki sab apni jagah mila liya gaya — sirf <b>{result.layout_penalty} marks</b> kate hain.
            </span>
          </div>
        </div>
      )}
