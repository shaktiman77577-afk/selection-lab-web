  ...
  if (stage === "test") {
    ...
    return (
      <Shell>
        ...
        {/* Pehle sawal par ek baar — har sawal par dohrana jagah khaata hai */}
        {idx === 0 && <div style={{ marginTop: 12 }}><PracticeNotice compact /></div>}
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 13, marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 14.5, fontWeight: 700, lineHeight: 1.6 }}>
            {q.instruction_en || q.instruction_hi}
          </div>
        ...
        {/* Hamesha dikhta hai — type karna band hote hi gaayab hota to neeche
            ke button khisak jaate aur phone par pehla tap khaali jaata. */}
        <div style={{ fontSize: 11.5, color: typing ? GOLD : "var(--muted)", marginTop: 6, lineHeight: 1.5 }}>
          While typing a formula, tap a cell to add its reference, or drag across cells to add a
          range (like C2:C7).
        </div>
              ...
              >
                ...
                {pt.fill_range && on && (
                  <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 4, lineHeight: 1.5 }}>
                    Press Enter, then drag the small square at the corner of the cell down to{" "}
                    <b style={{ color: "var(--text)" }}>{(pt.fill_range || "").split(":")[1]}</b>,
                    just as you do in Excel.
                  </div>
                )}
        ...
        return (
          <div key={d.question_id} style={{ background: "var(--card)", border: "1px solid var(--line)", borderLeft: `3px solid ${tone}`, borderRadius: 10, padding: 12, marginBottom: 10 }}>
            ...
            <div style={{ fontSize: 13, marginTop: 6, lineHeight: 1.55 }}>
              {d.instruction_en || d.instruction_hi}
            </div>
