...
export default function HomePage() {
  ...
  const [heroCfg, setHeroCfg] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    // Admin app-config hero slides — same source as the app's carousel.
    fetch("https://api.selectionlab.online/api/app-config")
      .then((r) => (r.ok ? r.json() : null))
      .then((cfg) => {
        const root = cfg?.config ?? cfg; // endpoint may wrap as { config: {...} }
        const hs = root?.hero_slides ?? root?.heroSlides ?? root?.slides;
        if (Array.isArray(hs)) setHeroCfg(hs);
      })
      .catch(() => {});
  }, []);

  // One unified slide list, exactly like the app: config hero slides first
  // (content or image), then admin promo banners. Everything (timer, dots,
  // swipe) derives from THIS list, so counts can never mismatch.
  type HeroS =
    | { kind: "content"; title: string; subtitle: string; emoji: string;
        pLabel: string; pAction: string; sLabel: string; sAction: string }
    | { kind: "image"; img: string; action: string; title: string };

  const slides: HeroS[] = [];
  for (const sd of heroCfg) {
    if (String(sd?.type ?? "content") === "image") {
      const img = String(sd?.image_url ?? "").trim();
      if (img) slides.push({ kind: "image", img, action: String(sd?.primary_action ?? ""), title: "" });
    } else {
      ...
      if (title)
        slides.push({
          kind: "content",
          title,
          subtitle: String(sd?.subtitle ?? ""),
          emoji: String(sd?.emoji ?? ""),
          pLabel: String(sd?.primary_label ?? ""),
          pAction: String(sd?.primary_action ?? ""),
          sLabel: String(sd?.secondary_label ?? ""),
          sAction: String(sd?.secondary_action ?? ""),
        });
    }
  }
  for (const b of banners) {
    const img = bannerImage(b);
    if (!img) continue; // imageless banners never join the carousel
    // DB me column `link_url` hai; purane/alag naam bhi handle kar lete hain
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "16px 16px 40px" }}>
        {/* ── Hero carousel (admin app-config slides + banners; falls back to default hero) ── */}
        {slides.length > 0 ? (
          <section
            ...
            {slides.map((s, i) => (
              <div
                ...
                onClick={() => {
                  ...
                  heroGo(s.kind === "image" ? s.action : s.pAction);
                }}
                style={{
                  position: "absolute",
                  inset: 0,
              ...
              >
                {s.kind === "image" ? (
                  <>
                    ...
                    />
                    {/* Poster me apna text pehle se hota hai, isliye overlay title nahi dikhate */}
                    {false && (
                      <div
                        style={{
                          position: "absolute",
                  ...
                  >
                    <div style={{ position: "absolute", right: -30, top: -30, width: 140, height: 140, borderRadius: "50%", background: "rgba(255,171,0,0.15)" }} />
                    {!!s.emoji && <div style={{ fontSize: 30, lineHeight: 1 }}>{s.emoji}</div>}
                    <h1 style={{ margin: "8px 0 0", fontSize: 22, fontWeight: 800, lineHeight: 1.3, maxWidth: 560 }}>
                      {s.title}
                    </h1>
                    {!!s.subtitle && (
                      <p style={{ margin: "8px 0 0", fontSize: 13.5, lineHeight: 1.55, color: "rgba(255,255,255,0.85)", maxWidth: 520 }}>
                        {s.subtitle}
                      </p>
                    )}
                    {(s.pLabel || s.sLabel) && (
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
                        {!!s.pLabel && (
                          <button
                            onClick={(e) => { e.stopPropagation(); heroGo(s.pAction); }}
                            style={{ ...goldBtn, padding: "10px 18px" }}
                          >
                            {s.pLabel}
                          </button>
                        )}
                        {!!s.sLabel && (
                          <button
                            onClick={(e) => { e.stopPropagation(); heroGo(s.sAction); }}
                            style={{ background: "rgba(255,255,255,0.12)", color: "#fff", border: "1px solid rgba(255,255,255,0.4)", borderRadius: 10, padding: "10px 18px", fontWeight: 800, fontSize: 13.5, cursor: "pointer" }}
                          >
                            {s.sLabel}
                          </button>
                        )}
