...
import { useOfficeAccess } from "@/lib/officeAccess";
...
export default function OfficeTaskPage() {
  ...
  // Pehla Word/PPT task free; baaki NBEMS series kharidne par
  if (!access.canOpen(task.id)) {
    return (
      <div style={{ maxWidth: 700, margin: "0 auto", padding: 20, color: "var(--text)" }}>
        <button onClick={() => router.push(`/office-practice${qs}`)} aria-label="All tasks"
          style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", color: "var(--text)", padding: 0 }}>←</button>
        {access.loading ? (
          <p style={{ color: "var(--muted)" }}>Loading…</p>
        ) : (
          <div style={{ background: "var(--card)", border: `1px solid ${GOLD}`, borderRadius: 14, padding: 18, marginTop: 10, textAlign: "center" }}>
            <div style={{ fontSize: 30 }}>🔒</div>
            <div style={{ fontSize: 16, fontWeight: 800, margin: "6px 0" }}>{task.title}</div>
            <p style={{ fontSize: 13, color: "var(--muted)", lineHeight: 1.6, margin: "0 0 14px" }}>
              The first Word and the first PowerPoint task are free. Unlock the NBEMS series to practise all the tasks.
            </p>
            <button onClick={() => router.push(access.unlockHref())}
              style={{ padding: "11px 20px", borderRadius: 10, border: "none", background: GOLD, color: "#1a1a1a", fontWeight: 800, cursor: "pointer" }}>
              Unlock
            </button>
          </div>
        )}
