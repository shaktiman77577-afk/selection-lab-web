...
type Tab = "home" | "health" | "seo" | "live" | "tickets" | "dashboard" | "courses" | "questions" | "qbank" | "mocktests" | "blog" | "banners" | "notifications" | "reviews" | "users" | "coupons" | "descriptive" | "appcontent" | "approvals" | "tier2" | "excel" | "search" | "email" | "partners" | "scorechecker" | "composer" | "extractor" | "volatile";
...
function QuestionsTab() {
  ...
  // Backend ek baar me 1000 se zyada nahi leta (badi request timeout ho jaati
  // hai). Isliye file todne ki zarurat nahi — yahi 1000-1000 ke tukdon me bhej
  // deta hai. Beech me koi tukda fail ho to wahi ruk jata hai aur bata deta hai
  // kitne chadh chuke, taaki dobara poori file na bhejni pade.
  const CHUNK = 1000;
  ...
  async function upload() {
    ...
    try {
      for (let i = 0; i < total; i += CHUNK) {
        const part = parsed.slice(i, i + CHUNK);
      ...
      setError(
          ...
          ? `${done} questions chadh chuke the, phir ruk gaya: ${e.message} — file me se pehli ${done} rows hata kar baaki dobara bhejiye`
          : e.message
      );
    }
    setUploading(false);
...
function QuestionBankTab() {
  ...
  // Upload ke baad sabse bada sawaal: "jo chadha hai wo dikhta kaisa hai?"
  // 5,000 questions ek-ek karke dekhna mumkin nahi. Ye har topic se 20 random
  // questions uthata hai aur wahi PDF banata hai jo mock review me banti hai —
  // answer aur explanation ke saath. Har baar alag sample aata hai.
  async function downloadSample() {
    setSamplePdf(true);
    try {
      const res = await fetch(`${API_URL}/qbank/sample-pdf?per_topic=20&max_questions=400`, {
    ...
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "0 0 10px", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
          ...
          <button onClick={downloadSample} disabled={samplePdf}
            title="Har topic se 20 random questions, asli paper jaise PDF me"
            style={{ ...ghostBtn, padding: "7px 12px", fontSize: 12, opacity: samplePdf ? 0.6 : 1 }}>
            {samplePdf ? "Ban rahi hai…" : "📄 Sample PDF"}
          </button>
        </div>
      </div>
