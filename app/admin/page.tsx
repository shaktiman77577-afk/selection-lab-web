...
function QualityReport({ rows, chartNames }: { rows: any[]; chartNames?: Set<string> }) {
  ...
  // Jaanch sirf tab dobara jab file ya chart list badle. Pehle ye har render
  // par chalti thi — 13,000+ rows ki file me upload ke har progress update
  // par poori jaanch dobara hoti thi aur phone ki screen atak jaati thi.
  const audit = useMemo(() => {
    ...
    return {
      issues, stats, badByRow,
      errors: issues.filter((i) => i.level === "error"),
      warns: issues.filter((i) => i.level === "warn"),
      badRows: Array.from(badByRow.keys()).sort((a, b) => a - b),
    };
  }, [rows, chartNames]);
  if (!audit) return null;
  const { issues, stats, errors, warns, badByRow, badRows } = audit;
...
function QuestionsTab() {
  ...
  // Dono sirf file/chart badalne par — upload ke progress par dobara nahi banti
  const chartNameSet = useMemo(() => new Set(charts.flatMap((f) => {
    const n = f.name.trim().toLowerCase();
    return [n, n.replace(/\.[a-z0-9]+$/, "")];
  })), [charts]);
  const chartsUsed = useMemo(() => {
    ...
    return charts.filter((f) => { const n = f.name.trim().toLowerCase(); return refs.has(n) || refs.has(noExt(n)); }).length;
  }, [charts, parsed]);
  ...
  async function upload() {
    ...
    if (charts.length) {
      try {
        ...
        // ── Chhote tukdon me ──
        // 20 file ya ~8 MB, jo pehle ho, aur ek waqt me sirf 2 tukde. Pehle 50
        // ke tukde 3 ek saath jaate the — mobile data par request beech me kat
        // jaati thi aur saare fail ho jaate the.
        ...
        // ── Server se poochho: ye naam pehle kabhi chadha hai kya ──
        // Page reload hone par browser ka apna chartCache khaali ho jaata
        // hai, isliye yahan DB se poochte hain — pehle se chadhi image
        // dobara ImgBB tak nahi jaati.
        try {
          const lookupRes = await fetch(`${API_URL}/uploads/lookup`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${localStorage.getItem("sl_admin_token") || ""}`,
            },
            body: JSON.stringify({ names: need.map((f) => f.name) }),
          });
          if (lookupRes.ok) {
          ...
          // lookup fail ho to koi baat nahi — bas normal upload chalega
        }
        ...
        let lastErr = "";
        let sent = 0;
        let stopWhy = "";          // bhara hua = aage ke tukde mat bhejo
        let deadStreak = 0;        // lagataar kitne tukde poore ke poore fail
        const total = todo.length;
        const cached = need.length - todo.length;
        // Asli wajah progress ke saath hi dikhti hai, upload khatam hone ka intezaar nahi
        const progress = () => setResult(
          `Charts upload ho rahe hain… ${sent}/${total}` +
          (cached ? ` (${cached} pehle hi chadh chuke)` : "") +
          (failed.length ? ` · ${failed.length} fail — ${lastErr || "?"}` : "")
        );
        if (total) progress();

        // Ek tukda bhejo. Laut ta hai: kitni files chadhi.
        const sendBatch = async (b: File[]): Promise<number> => {
          for (let attempt = 1; attempt <= 2; attempt++) {
            ...
            try {
              ...
              let res: Response;
              try {
                res = await fetch(`${API_URL}/uploads/bulk-images`, {
                  method: "POST",
                ...
                throw Object.assign(
                  new Error(e?.name === "AbortError"
                    ? "3 minute me server ka jawab nahi aaya (internet dheema ya server busy)"
                    : `Server tak nahi pahunche (${String(e?.message || e)})`),
                  { retry: true });
              }
              const d = await res.json().catch(() => ({} as any));
              if (!res.ok) {
                const msg = `${d.detail || "Server error"} (HTTP ${res.status})`;
                // Login / key / request ki galti — dobara bhejne se kuch nahi badlega
                const fatal = res.status === 401 || res.status === 403 || res.status === 503
                  || (res.status >= 400 && res.status < 500 && res.status !== 408 && res.status !== 429);
                if (fatal) stopWhy = msg;
                throw Object.assign(new Error(msg), { retry: !fatal });
              }
              ...
              let ok = 0;
              b.forEach((f, i) => {
                const r = results.length === b.length ? results[i] : null;
                const u = (r && r.ok && r.url) || urlMap[f.name] || urlMap[f.name.trim()];
                if (u) { chartCache.current.set(key(f), String(u)); ok++; }
                else {
                  failed.push(f.name);
              ...
              if (attempt === 2 || !e?.retry || stopWhy) {
                b.forEach((f) => failed.push(f.name));
                return 0;
              }
        ...
        let next = 0;
        const worker = async () => {
          while (next < batches.length && !stopWhy) {
            const b = batches[next++];
            const ok = await sendBatch(b);
            sent += b.length;
            // Lagataar 3 tukde poore fail = gadbad har file me hai (internet,
            // server, ImgBB). Baaki hazaaron ko bhi fail karwane ka fayda nahi.
            deadStreak = ok === 0 ? deadStreak + 1 : 0;
            if (deadStreak >= 3 && !stopWhy) stopWhy = "lagataar 3 tukde poore fail hue";
            progress();
          }
        };
        await Promise.all(Array.from({ length: PARALLEL }, () => worker()));

        if (failed.length || stopWhy) {
          const ok = sent - failed.length;
          throw new Error(
            (stopWhy ? `Upload rok diya — ${stopWhy}. ` : "") +
            `${failed.length} charts nahi chadhe` +
            (failed.length ? ` (${failed.slice(0, 5).join(", ")}${failed.length > 5 ? "…" : ""})` : "") +
            (lastErr ? `. Wajah: ${lastErr}` : "") +
            `. ${Math.max(0, ok) + cached} chadh chuke hain, ${total - sent} abhi bheje hi nahi. ` +
            `Upload dobara dabaiye — jo chadh chuke wo dobara nahi jayenge`
          );
        }
        ...
        {parsed.length > 0 && !parseErr && (() => {
          ...
          return (
            <div style={{ marginTop: 12 }}>
              ...
              {withImg.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
                    {/* Sirf asli link dikhate hain — sirf filename wale (M7-Q30.png) abhi upload hone baaki hain, unka preview toota dikhta tha */}
                    {Array.from(new Set(withImg.map((p: any) => String(p.image_url).trim())))
                      .filter((u) => /^https?:\/\//i.test(u)).slice(0, 8).map((u: any) => (
                      <img
                        key={u}
                        src={u}
                        alt=""
              ...
              <QualityReport rows={parsed} chartNames={chartNameSet} />
            </div>
          );
        })()}
