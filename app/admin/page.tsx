...
type Tab = "home" | "health" | "seo" | "live" | "tickets" | "dashboard" | "courses" | "questions" | "qbank" | "mocktests" | "blog" | "banners" | "notifications" | "reviews" | "users" | "coupons" | "descriptive" | "appcontent";
...
const BULLET_KB: { keys: string[]; a: string }[] = [
  { keys: ["csv", "format", "column", "columns"],
    a: "Aapne sahi cheez poochhi. CSV ka format ye raha:\n1-5: Question, A, B, C, D (English)\n6: Answer (sirf A/B/C/D)\n7-8: Explanation, Topic\nHindi bhi chahiye to 9-14 me wahi cheezein Hindi me, 15th me Section ka naam, aur 16th me chart/diagram ka image link (DI ke liye). Sheets se paste kijiye, TAB apne aap chal jaata hai." },
      ...
      {tab === "home" ? (
        ...
        <div style={{ padding: "20px 16px 140px" }}>
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 24 }}>🎫</span>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 800, fontSize: 15, color: GOLD }}>Support tickets</div>
                <div style={{ fontSize: 12, color: "#9a917f" }}>Students ke sawaal aur unke jawab</div>
              </div>
        ...
        <div style={{ padding: "14px 16px 0", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
            <span style={{ fontSize: 18 }}>{activeAction?.icon || (tab === "health" ? "🩺" : tab === "seo" ? "📈" : tab === "live" ? "🟢" : tab === "tickets" ? "🎫" : "📊")}</span>
            <span
              style={{
            ...
            >
              {activeAction?.title || (tab === "health" ? "Troubleshooter" : tab === "seo" ? "SEO Coach" : tab === "live" ? "Live activity" : tab === "tickets" ? "Support tickets" : "Aaj ki report")}
            </span>
          </div>
        </div>
      )}
      ...
      {tickets.map((t) => (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 11, flexWrap: "wrap" }}>
            ...
            {t.user_id && (
              <button onClick={() => alert(`User ID: ${t.user_id}\nUsers tab me search karke access de sakte hain.`)} style={{ ...smallBtn, padding: "7px 14px" }}>
                User #{t.user_id}
              </button>
            )}
            {t.phone && (
    ...
    <div>
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 14, marginBottom: 14 }}>
        ...
        <p style={{ margin: "0 0 12px", fontSize: 12.5, color: "#9a917f", lineHeight: 1.6 }}>
          8 columns: Question, A, B, C, D, Answer (A/B/C/D), Explanation, Topic. Optional: +6 Hindi columns (Question, A, B, C, D, Explanation in Hindi), then column 15 = Section name (e.g. General Awareness), and column 16 = Image URL for DI charts, tables or diagrams. For a DI set where 5 questions share one chart, put the same link in all 5 rows.
...
function MockTestsTab() {
  ...
  async function saveChartImg(qId: number) {
    ...
    setChartErr("");
    try {
      await api(`/admin-extra/questions/${qId}`, "PATCH", { image_url: chartImgDraft.trim() || null });
      setChartQs((prev) => prev.map((q) => (q.id === qId ? { ...q, image_url: chartImgDraft.trim() || null } : q)));
      setEditingChartQId(null);
    } catch (e: any) {
      setChartErr(e.message);
    }
...
function BannersTab() {
  ...
  const [form, setForm] = useState<any>({ image_url: "", image_url_mobile: "", title: "", link_url: "", display_order: "0" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  async function add() {
    ...
    try {
      ...
      setForm({ image_url: "", image_url_mobile: "", title: "", link_url: "", display_order: String(banners.length + 1) });
      load();
