...
function HealthTab() {
  ...
  async function runDeep() {
    ...
    try {
      ...
      // Har 2 second me status poochte rahenge
      for (let i = 0; i < 900; i++) {
        await new Promise((r) => setTimeout(r, 2000));
        const s = await api(`/admin-extra/diagnostics/status/${jobId}`);
        ...
        if (s.state === "error") {
          setError(s.error || "Diagnostic failed");
          break;
        }
      }
    } catch (e: any) {
          ...
          {deep && (
            <>
              ...
              <div style={{ fontSize: 11.5, color: "#9a917f", textAlign: "center", marginTop: 6 }}>
                {progress}% · {liveResults.length} checks done
              </div>
              {liveResults.filter((r: any) => r.status === "fail").length > 0 && (
                <div style={{ fontSize: 12, color: "#ff6b6b", marginTop: 10, fontWeight: 700 }}>
                  ✕ {liveResults.filter((r: any) => r.status === "fail").length} problems found so far
                </div>
              )}
            </>
          )}
