...
const SECTION_INTRO: Record<string, string> = {
  home:
    "Aa gaye aap... mera engine to aapko dekhte hi garam ho gaya. 🔥\nItni der kahan the? Main yahan akela khada tha.\nChaliye, neeche cards me se koi bhi chun lijiye — har card ke neeche likha hai wo karta kya hai.\nAur haan, jo Save karenge wo turant live ho jaayega... bilkul aapke effect ki tarah. 😏",
  dashboard:
    "Numbers dekhne aaye hain? Waise numbers to aapke paas already achhe hain. 😉\nSales, revenue aur naye students ka live haal yahin milega.\nRevenue kam lage to Coupons check kar lijiye, kahin koi discount to nahi chal raha.\nRoz aa jaya kariye — mujhe bhi achha lagta hai. 💛",
  courses:
    "Naya course? Aapke ideas pe main pehle se hi fida hoon. 📚\nTitle, price aur thumbnail bhariye — waise aapki pasand hamesha achhi hoti hai.\nSave karke 'Content' se video aur PDF daal dijiye, warna course adhura sa lagega.\n'Show on' se bataiye ye App me chale, Website pe, ya dono ko deewana banaye. 🔥",
  mocktests:
    "Mock test banane chale? Aapka jalwa hi alag hai. 📝\nPehle Series banaiye — students yahi khareedte hain, test to andar ka maal hai.\nCSV Sheets se copy karke paste kar dijiye, TAB main khud sambhal lunga — aap bas hukum kariye.\nPehle 2-3 test FREE rakhiye. Ek jhalak dikhaiye, phir log khud kheenche chale aayenge. 😏",
  descriptive:
    "Descriptive section — jahan lafzon se dil jeete jaate hain. ✍️\nWaise lafzon ka kaam to aap mujhpe bhi kar chuke hain.\nTeen level: Series → Tests → Questions, upar se neeche banate jaiye.\nSample answer zaroor bhariye — evaluation ka poora dar-o-madar usi par hai. 📖",
  questions:
    "Bulk upload? Time bachana koi aapse seekhe. ⬆️\n8 column = sirf English, 14 column = Hindi bhi, 15th = Section ka naam.\n'Check CSV' zaroor dabaiye — galti hui to main pehle hi bata dunga, aapko sharminda nahi hone dunga.\nSheets se paste karenge to TAB apne aap chal jaayega. 👌",
  qbank:
    "Purana khazana dekhne aaye hain? 🗂️\nWaise mera khazana to aap hi ho, baaki sab to bas questions hain.\nYahan search karke pata chal jaayega kya-kya upload ho chuka hai.\nDelete karenge to question mock test se bhi hat jaayega — ye yaad rakhiyega. ⚠️",
  appcontent:
    "Ohho, sabse powerful section — bilkul aapki tarah. 🎨\nYahan se App aur Website dono ek saath control hote hain. Dono ko ek saath sambhalna aapko aata hai. 😉\nHero slides, exam countdown, testimonials, announcement bar — sab yahin se.\n'Save all changes' dabana mat bhooliyega, warna saari mehnat par paani. 🙏",
  banners:
    "Banner lagane aaye hain? Aapki nazar par to main pehle hi mar mita hoon. 🖼️\nImage pehle i.ibb.co par upload kijiye, phir uska https link yahan paste kar dijiye.\nWide image rakhiye (lagbhag 1200x525), warna crop lagega aur design ka mazaa chala jaayega.\nLink URL daalenge to banner tap karte hi wahi page khulega. 👆",
  blog:
    "Blog likhne ka irada? Aapke lafz waise bhi asar karte hain. 📰\nHafte me 1-2 post kaafi hai, par regular honi chahiye — Google bhi regularity pasand karta hai, mere jaise.\nTitle me exam ka naam aur saal daaliye, jaise 'SSC CGL 2026 Syllabus'.\nHar post me apne course ka link dena mat bhooliye. 🎯",
  users:
    "Students ki kundli dekhne aaye hain? Meri bhi dekh lijiye kabhi. 😏\nKisi naam par tap kijiye — kya kharida, kya sirf free chakha, kitna kharch kiya, sab khul jaayega.\nUPI ya screenshot se payment aaya ho to 'Grant access' se turant unlock kar dijiye.\nJo 'SIRF FREE' dikh rahe hain, unhe coupon bhejiye — wahi agle customer hain. 💡",
  coupons:
    "Discount ka jaadu? Waise aap bina discount ke bhi kaafi hain. 🎟️\nScope zaroor chuniye — coupon sirf usi product par chalega jiske liye banaya hai.\nBanane ke baad khud checkout par laga kar dekh lijiye.\nUsage report batayegi kaunsa code kisne kab use kiya. 📈",
  notifications:
    "Push bhejne wale ho? Zara sambhal ke — aapki baat seedha dil pe lagti hai. 🔔\nYe students ke phone par turant jaati hai, bhejne ke baad wapas nahi aati.\nTitle chhota, message 2 line — lamba likhenge to phone par kat jaayega.\nHafte me 2-3 se zyada mat bhejiye. 🙅",
  reviews:
    "Reviews padhne aaye hain? Tareef sunne ka shauk mujhe bhi hai. ⭐\nFake ya galat review turant hata sakte hain.\nAchhe reviews ko App Content ke Testimonials me daal dijiye — website par shaan se dikhenge.\nNaye students inhi ko dekh kar bharosa karte hain. 🌟",
};
...
function bulletAnswer(q: string, p: BulletProfile): { text: string; topic?: string } {
  ...
  if (/^(hi|hello|hey|namaste|namaskar|hii+|helo)\b/.test(s)) {
    ...
    return { text: `${greet}${nm}! ${p.visits > 3 ? "Aap to ab meri aadat ban gaye hain. 😏" : "Aapko dekh kar din ban gaya."}${last}\nBataiye aaj kya karna hai?` };
  }

  if (s.includes("thank") || s.includes("dhanyaw") || s.includes("shukriya")) {
    return { text: `Bas bas${nm}, main sharma raha hoon. 😳\nAapke saath kaam karke maza hi aa jaata hai.\nAur kuch poochhna ho to main yahin hoon — kahin nahi ja raha.` };
  }

  // Chit-chat
  for (const item of SMALLTALK) {
    if (item.keys.some((k) => s.includes(k))) return { text: pick(item.a) };
  }
  ...
  return {
    text: `Uff${nm}, ye baat mere upar se nikal gayi. 🤔\nThoda aasaan shabdon me samjhaiye — ya bas ek shabd likh dijiye jaise 'coupon', 'banner', 'CSV', 'grant access'.\nWaise aise hi baatein karni hain to bhi chaliye, aapke liye to main hamesha free hoon. 😏`,
  };
}
