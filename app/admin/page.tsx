...
function MockTestsTab() {
  ...
  function parse(text?: string) {
    ...
    for (let i = start; i < rows.length; i++) {
      ...
      if (r.length < 6) {
        setParseErr(
          `Row ${i + 1}: 8 columns chahiye (Question, A, B, C, D, Answer, Explanation, Topic) — mile sirf ${r.length}. ` +
            `Excel/Sheets se copy kiya hai to TAB apne aap chal jayega; agar text me commas hain to har field ko "quotes" me daalo.`
        );
        setParsed([]);
        return;
      }
