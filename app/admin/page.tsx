...
function auditRows(rows: any[]): { issues: Issue[]; stats: Record<string, number> } {
  ...
  rows.forEach((q, idx) => {
    ...
    if (!imgCols[0] && !imgCols[1] && wantsChart.test(qt)) {
      stats.chartNotGiven++;
...
function QuestionsTab() {
  ...
  async function upload() {
    ...
    if (charts.length) {
      try {
        setResult(`${charts.length} charts upload ho rahe hain…`);
        const fd = new FormData();
        charts.forEach((f) => fd.append("files", f));
        ...
        const d = await res.json();
        if (!res.ok) throw new Error(d.detail || `Charts upload fail (${res.status})`);
        ...
        Object.entries(d.url_map || {}).forEach(([k, v]) => {
          const name = String(k).trim().toLowerCase();
          map.set(name, String(v));
          baseMap.set(name.replace(/\.[a-z0-9]+$/, ""), String(v));
        });
        imgUp = d.uploaded || 0;
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 14, marginBottom: 14 }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <Field label="Charts / diagrams (Math aur Reasoning ke liye)"
            hint="TSV me sirf file ka naam likhiye (M7-Q30-Clock.png). Yahan wahi images chun lijiye — upload par naam se apne aap jud jayengi, question me bhi aur explanation me bhi.">
            <input type="file" accept="image/*" multiple
              onChange={(e) => setCharts(Array.from(e.target.files || []))}
              style={{ ...inputStyle, padding: "9px", fontSize: 13 }} />
          </Field>
          {charts.length > 0 && (
            <div style={{ fontSize: 11.5, color: "#5dd97c", marginTop: 4 }}>
              {charts.length} images chuni — TSV se pehle ye upload hongi
            </div>
          )}
        </div>
