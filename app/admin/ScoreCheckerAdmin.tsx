    ...
    <div>
      ...
      {exams.map((e) => (
        <ExamCard
          ...
          exam={e}
          api={api}
          sections={sections.filter((x) => x.exam_id === e.id)}
          onChange={load}
          onMsg={setMsg}
          onErr={setErr}
        />
...
function ExamCard({
  ...
  async function questionsPdf(section?: string) {
    setBusy(section ? `q:${section}` : "q:all");
    try {
      const qs = section ? `?section=${encodeURIComponent(section)}` : "";
      const res = await fetch(
        `${API_URL}/score-checker-admin/exams/${exam.id}/questions-pdf${qs}`,
      ...
      const dupes = Number(res.headers.get("X-Duplicates-Removed") || 0);
      const missing = Number(res.headers.get("X-English-Missing") || 0);

      await downloadBlob(await res.blob(), filenameFrom(res, "questions.pdf"));

      let note = `${count} questions from ${shiftCount} shift(s).`;
      if (dupes) note += ` ${dupes} repeated question(s) skipped.`;
      if (missing) note += ` English missing in ${missing}.`;
      onMsg(note);
    } catch (e: any) {
      onErr(e.message);
    } finally {
  ...
  return (
    <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 14, marginBottom: 12 }}>
      ...
      {/* Question papers — sc_answer_keys se bante hain, digialm se nahi.
          Isliye ye tab bhi chalte hain jab links marr chuke hon. */}
      {sections.length > 0 && (
        <div style={{ marginBottom: 11 }}>
          <div style={{ fontSize: 11, color: MUTED, marginBottom: 6 }}>
            Question papers (all shifts together)
          </div>
          <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
            <button
              onClick={() => questionsPdf()}
              disabled={!!busy}
              style={{ ...btn, borderColor: GOLD, color: GOLD }}
            >
              {busy === "q:all"
                ? "..."
                : `All questions (${sections.reduce((a, x) => a + x.questions, 0)})`}
            </button>
            {sections.map((sec) => (
              <button
                key={sec.name}
                onClick={() => questionsPdf(sec.name)}
                disabled={!!busy}
                style={{ ...btn, borderColor: GOLD, color: GOLD }}
              >
                {busy === `q:${sec.name}` ? "..." : `${sec.name} (${sec.questions})`}
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Data export */}
      <div style={{ fontSize: 11, color: MUTED, marginBottom: 6 }}>Data export</div>
      <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
