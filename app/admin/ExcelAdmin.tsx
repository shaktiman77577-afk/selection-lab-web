...
function ChartTab({ api, seriesId }: { api: ApiFn; seriesId: number }) {
  ...
  async function remove(c: any) {
    if (!confirm(`"${c.name}" card hata dein?`)) return;
      ...
      {cards.map((c) => (
        <div key={c.id} style={rowCard}>
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={() => remove(c)} style={dangerBtn}>Hide</button>
          </div>
        </div>
      ))}
    </div>
    ...
    <div>
      ...
      {rows.map((q, i) => (
        <div key={q.id} style={rowCard}>
          ...
          <button onClick={() => remove(q)} style={dangerBtn}>Hide</button>
        </div>
      ))}
  ...
  return (
    <div>
      ...
      {rows.map((q, i) => (
        <div key={q.id} style={rowCard}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{i + 1}. {q.question_en}</div>
