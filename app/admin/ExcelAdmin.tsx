...
function WorksheetTab({ api, seriesId }: { api: ApiFn; seriesId: number }) {
  ...
  function edit(t: any) {
    ...
    setSheet({ cells, bold: t.bold_cells || [], merges: t.merges || [],
               // Phase 2C: sample ki formatting — student ki sheet isi se jaanchi jaati hai
               borders: t.border_cells || [], align: t.align_cells || {},
               wrap: t.wrap_cells || [], fmt: t.num_formats || {} });
  ...
  async function save() {
    ...
    const body = {
      ...
      border_cells: sheet.borders || [], align_cells: sheet.align || {},
      wrap_cells: sheet.wrap || [], num_formats: sheet.fmt || {},
  ...
  return (
    <div>
      <div style={{ ...cardBox, marginBottom: 16 }}>
        ...
        {/* Phase 2C — jo formatting yahan lagegi, wahi student se maangi jaayegi */}
        <div style={{ fontSize: 11.5, color: "#9a917f", lineHeight: 1.6, marginBottom: 8 }}>
          Sheet bilkul question paper jaisi banaiye: poori table chun kar <b>▦ Borders</b>,
          heading par Merge + ↔, lambe text par Wrap, % / decimal wale column par % ya .0+.
          Jo yahan lagega wahi student se jaancha jaayega. <b>Border lagaya to student ki sheet
          border ke bina 0 paayegi</b> (asli exam jaisa). Border nahi lagaya to purana niyam.
        </div>
        <div style={{ background: "#fff", borderRadius: 8, padding: 8, marginBottom: 14 }}>
          {/* advanced = row/column jodne-hatane ke button. Sirf admin ko —
              student ko sheet ka dhancha badalna nahi hota, use to wo
              banani hai jo kagaz par chhapi hai. */}
          <SheetEditor rows={14} cols={10} value={sheet} onChange={setSheet}
                       computed={computed} advanced />
        </div>
