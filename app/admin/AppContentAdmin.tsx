...
export default function AppContentAdmin({ api }: { api: ApiFn }) {
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      ...
      {/* EXAM DATES (website countdown) */}
      <div style={box}>
        <div style={head}>📅 Exam Dates (website countdown)</div>
        {(cfg.exam_dates || []).map((e: any, i: number) => (
          <div key={i} style={card}>
            <div style={{ ...row, justifyContent: "space-between" }}>
              <b style={{ fontSize: 12, color: TEXT }}>Exam {i + 1}</b>
              <button style={delBtn} onClick={() => removeItem("exam_dates", i)}>Delete</button>
            </div>
            <span style={lbl}>Exam name</span>
            <Field value={e.name} onChange={(v) => updateItem("exam_dates", i, { name: v })} placeholder="SSC CGL Tier 1" />
            <span style={lbl}>Exam date</span>
            <input
              type="date"
              style={input}
              value={e.date || ""}
              onChange={(ev) => updateItem("exam_dates", i, { date: ev.target.value })}
            />
          </div>
        ))}
        <button style={darkBtn} onClick={() => addItem("exam_dates", { name: "", date: "" })}>+ Add exam date</button>
        <div style={{ fontSize: 11, color: MUTED, marginTop: 6 }}>Website homepage pe live countdown dikhta hai. Purani dates apne aap hide ho jaati hain.</div>
      </div>

      {/* TESTIMONIALS (website Selection Stories) */}
      <div style={box}>
        <div style={head}>🏆 Testimonials (Selection Stories)</div>
        {(cfg.testimonials || []).map((t: any, i: number) => (
          <div key={i} style={card}>
            <div style={{ ...row, justifyContent: "space-between", marginBottom: 6 }}>
              <b style={{ fontSize: 12, color: TEXT }}>Story {i + 1}</b>
              <div style={row}>
                <button style={darkBtn} onClick={() => move("testimonials", i, -1)}>↑</button>
                <button style={darkBtn} onClick={() => move("testimonials", i, 1)}>↓</button>
                <button style={delBtn} onClick={() => removeItem("testimonials", i)}>Delete</button>
              </div>
            </div>
            <div style={row}>
              <div style={{ flex: 1 }}>
                <span style={lbl}>Student name</span>
                <Field value={t.name} onChange={(v) => updateItem("testimonials", i, { name: v })} placeholder="Rahul Kumar" />
              </div>
              <div style={{ flex: 1 }}>
                <span style={lbl}>Exam / result</span>
                <Field value={t.exam} onChange={(v) => updateItem("testimonials", i, { exam: v })} placeholder="SSC CGL 2025 — Selected" />
              </div>
            </div>
            <span style={lbl}>Quote</span>
            <Field area value={t.quote} onChange={(v) => updateItem("testimonials", i, { quote: v })} placeholder="Mock tests bilkul asli exam jaise the…" />
            <span style={lbl}>Photo URL (optional — khali chhodo to initials dikhenge)</span>
            <Field value={t.image_url} onChange={(v) => updateItem("testimonials", i, { image_url: v })} placeholder="https://…/photo.jpg" />
          </div>
        ))}
        <button style={darkBtn} onClick={() => addItem("testimonials", { name: "", exam: "", quote: "", image_url: "" })}>+ Add story</button>
      </div>

      {/* ANNOUNCEMENT BAR (website top bar) */}
      <div style={box}>
        <div style={head}>📢 Announcement Bar</div>
        <div style={{ ...row, marginBottom: 10 }}>
          <button
            style={(cfg.announcement || {}).enabled ? goldBtn : darkBtn}
            onClick={() => setField("announcement", { ...(cfg.announcement || {}), enabled: true })}
          >
            On
          </button>
          <button
            style={!(cfg.announcement || {}).enabled ? goldBtn : darkBtn}
            onClick={() => setField("announcement", { ...(cfg.announcement || {}), enabled: false })}
          >
            Off
          </button>
        </div>
        <span style={lbl}>Text</span>
        <Field
          value={(cfg.announcement || {}).text}
          onChange={(v) => setField("announcement", { ...(cfg.announcement || {}), text: v })}
          placeholder="🎉 Naya IB SA Live Batch shuru — aaj hi join karo!"
        />
        <span style={lbl}>Link (optional — "Dekho →" is link pe jayega)</span>
        <Field
          value={(cfg.announcement || {}).link}
          onChange={(v) => setField("announcement", { ...(cfg.announcement || {}), link: v })}
