...
// Per-type defaults + scoring hints shown in the admin form when a Type is picked.
const TYPE_META: Record<string, { wl: number; wlLabel: string; marks: [number, number, number]; hint: string }> = {
  Essay: {
    wl: 250, wlLabel: "Word limit (target)", marks: [5, 10, 10],
    hint: "Scored on closeness to the target word count, plus spelling & grammar.",
  },
  Letter: {
    wl: 150, wlLabel: "Word limit (target)", marks: [5, 10, 10],
    hint: "Format-checked: salutation + subject line + closing (70%) and length (30%), plus spelling & grammar. Raw word count matters less here.",
  },
  Precis: {
    wl: 100, wlLabel: "Ideal précis length (≈ 1/3 of passage)", marks: [5, 10, 10],
    hint: "Set the length to the IDEAL précis length (about one-third of the passage — e.g. 100 for a 300-word passage). Too long OR too short loses marks. Put the FULL passage in the Question box.",
  },
  Translation: {
    wl: 150, wlLabel: "Expected English length", marks: [10, 10, 10],
    hint: "Put the Hindi passage in the Question box. Scored on length × whether the answer is actually English (leaving Hindi text loses marks), plus spelling & grammar.",
  },
};
...
function SeriesLevel(props: {
  ...
  function startEdit(s: any) {
    setEditId(s.id);
    setForm({
      title: s.title || "", description: s.description || "", thumbnail_url: s.thumbnail_url || "", thumbnail_url_mobile: s.thumbnail_url_mobile || "",
  ...
  async function save() {
    ...
    try {
      const body: any = {
        title: form.title, description: form.description, thumbnail_url: form.thumbnail_url, thumbnail_url_mobile: String(form.thumbnail_url_mobile || "").trim() || null,
      ...
      {showForm ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          ...
          <ImageField
            ...
            reqW={1280}
            reqH={720}
            where="Descriptive list ka card, computer par"
          />
          ...
          <ImageField
            ...
            reqW={1080}
            reqH={1080}
            where="Wahi card phone par. Khaali chhodenge to desktop wali hi chalegi."
          />
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Field label="Telegram group link">
              <input style={inputStyle} placeholder="https://t.me/your_group_link" value={form.telegram_group || ""} onChange={(e) => setForm({ ...form, telegram_group: e.target.value })} />
            </Field>
            <Field label="Show on (kahan dikhe)">
              <select style={inputStyle} value={form.visible_on || "both"} onChange={(e) => setForm({ ...form, visible_on: e.target.value })}>
          ...
          <div key={s.id} style={rowCard}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 14 }}>
                {s.title}{" "}
                {!s.is_active ? <span style={{ fontSize: 10, color: MUTED, border: `1px solid ${BORDER}`, borderRadius: 6, padding: "1px 6px", marginLeft: 4 }}>INACTIVE</span> : null}
...
function TestsLevel(props: {
  ...
  function startEdit(t: any) {
    setEditId(t.id);
    setForm({ title: t.title || "", duration_min: t.duration_min ?? 30, is_free: !!t.is_free, display_order: t.display_order ?? 0 });
    setShowForm(true);
  ...
  async function save() {
    ...
    try {
      ...
      if (editId != null) await api(`/admin-extra/desc/tests/${editId}`, "PUT", body);
      else await api("/admin-extra/desc/tests", "POST", body);
      setForm(blank()); setShowForm(false); setEditId(null); reload();
  ...
  return (
    <div>
      <button onClick={() => { setShowForm(!showForm); if (showForm) { setEditId(null); setForm(blank()); } }} style={{ ...goldBtn, marginBottom: 14 }}>{showForm ? "Close form" : "+ Add test"}</button>

      {showForm ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          <h3 style={{ margin: "0 0 12px", fontSize: 15 }}>{editId != null ? "Edit test" : "New test"}</h3>
          <Field label="Title"><input style={inputStyle} value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></Field>
          <div style={{ display: "flex", gap: 10 }}>
          ...
          <button onClick={save} disabled={busy} style={{ ...goldBtn, width: "100%" }}>{busy ? "Saving…" : editId != null ? "Save changes" : "Create test"}</button>
        </div>
      ) : null}
...
function QuestionsLevel(props: {
  ...
  function startEdit(q: any) {
    setEditId(q.id);
    setForm({
      q_type: q.q_type || "Essay", question: q.question || "",
      word_limit: q.word_limit ?? 250, sample_answer: q.sample_answer || "",
      explanation: q.explanation || "",
      max_word_marks: q.max_word_marks ?? 5, max_spelling_marks: q.max_spelling_marks ?? 10,
      max_grammar_marks: q.max_grammar_marks ?? 10, display_order: q.display_order ?? 0,
    });
    setShowForm(true);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  // count existing questions per type (to enforce exactly 2 for Essay & Letter)
  const REQUIRE_TWO = ["Essay", "Letter"];
  function typeCount(t: string) {
    return (list || []).filter((q: any) => (q.q_type || "Essay") === t).length;
  }

  async function save() {
    if (!form.question.trim()) return alert("Question text is required.");
    // when editing, don't count the question being edited itself
    const effCount = typeCount(form.q_type) - (editId != null && (list || []).some((q: any) => q.id === editId && (q.q_type || "Essay") === form.q_type) ? 1 : 0);
    if (REQUIRE_TWO.includes(form.q_type) && effCount >= 2) {
      return alert(`${form.q_type} must have exactly 2 topics (students choose one). You already have 2 — delete one first to change it.`);
    }
    setBusy(true);
    try {
      const body = {
      ...
      if (editId != null) await api(`/admin-extra/desc/questions/${editId}`, "PUT", body);
      else await api("/admin-extra/desc/questions", "POST", body);
      setForm(blank()); setShowForm(false); setEditId(null); reload();
  ...
  return (
    <div>
      <button onClick={() => { setShowForm(!showForm); if (showForm) { setEditId(null); setForm(blank()); } }} style={{ ...goldBtn, marginBottom: 14 }}>{showForm ? "Close form" : "+ Add question"}</button>

      {showForm ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          <h3 style={{ margin: "0 0 12px", fontSize: 15 }}>{editId != null ? "Edit question" : "New question"}</h3>
          <div style={{ display: "flex", gap: 10 }}>
            <Field label="Type">
              <select style={inputStyle} value={form.q_type} onChange={(e) => {
                ...
                setForm({
                  ...
                  ...(m ? {
                    word_limit: m.wl,
                    max_word_marks: m.marks[0],
                    max_spelling_marks: m.marks[1],
                    max_grammar_marks: m.marks[2],
                  } : {}),
                });
              }}>
            ...
            <Field label={TYPE_META[form.q_type]?.wlLabel || "Word limit (target)"}><input type="number" style={inputStyle} value={form.word_limit} onChange={(e) => setForm({ ...form, word_limit: e.target.value })} /></Field>
          </div>
          {TYPE_META[form.q_type] ? (
            <div style={{ fontSize: 11.5, color: MUTED, background: "#0d0b08", border: `1px solid ${BORDER}`, borderRadius: 8, padding: "8px 10px", marginBottom: 10, lineHeight: 1.5 }}>
              ℹ️ <b style={{ color: GOLD }}>{form.q_type}:</b> {TYPE_META[form.q_type].hint}
            </div>
          ) : null}
          ...
          <Field label="Display order"><input type="number" style={inputStyle} value={form.display_order} onChange={(e) => setForm({ ...form, display_order: e.target.value })} /></Field>
          <button onClick={save} disabled={busy} style={{ ...goldBtn, width: "100%" }}>{busy ? "Saving…" : editId != null ? "Save changes" : "Create question"}</button>
        </div>
      ) : null}

      {/* Section requirements — Essay & Letter need exactly 2 topics (choose-one) */}
      <div style={{ ...cardBox, marginBottom: 12, padding: 12 }}>
        <div style={{ fontSize: 12, fontWeight: 800, marginBottom: 8, color: GOLD }}>Section requirements</div>
        {[
          { t: "Essay", need: "exactly 2", ok: typeCount("Essay") === 2 },
          { t: "Letter", need: "exactly 2", ok: typeCount("Letter") === 2 },
          { t: "Precis", need: "1", ok: typeCount("Precis") >= 1 },
          { t: "Translation", need: "1", ok: typeCount("Translation") >= 1 },
        ].map((r) => (
          <div key={r.t} style={{ fontSize: 12.5, color: r.ok ? "#4ec77a" : "#e0a13a", marginBottom: 3 }}>
            {r.ok ? "✓" : "⚠️"} {r.t}: {typeCount(r.t)} added{r.ok ? "" : ` — need ${r.need}`}
          </div>
        ))}
        {(typeCount("Essay") !== 2 || typeCount("Letter") !== 2) ? (
          <div style={{ fontSize: 11.5, color: MUTED, marginTop: 6 }}>
            Essay &amp; Letter must have 2 topics each so students can choose one. Précis &amp; Translation take a single passage.
          </div>
        ) : null}
      </div>
          ...
          <div key={q.id} style={{ ...cardBox, marginBottom: 10 }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "flex-start" }}>
              ...
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                ...
                <button onClick={() => remove(q)} style={dangerBtn}>Delete</button>
              </div>
            </div>
          </div>
        ))
