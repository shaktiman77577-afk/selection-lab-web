...
export default function Tier2Admin({ api }: { api: ApiFn }) {
  ...
  return (
    <div>
      ...
      {level === "series" && !loading && <RecomputePanel api={api} />}
    </div>
  );
}

// ── Purane typing results dobara jodna ─────────────────────────────────────
//
// Scoring me tail-detection ka bug tha: jo hissa student ne type hi nahi kiya
// tha wo poora "mistake" gin jata tha. Jis attempt me passage adhoora raha
// (yaani lagbhag har attempt) uska result 0% accuracy, 0 net WPM dikhata tha.
//
// Bug theek ho chuka hai, par database me pade purane results aaj bhi galat
// hain. Ye button unhe stored typed_text se dobara score karta hai.
//
...
function RecomputePanel({ api }: { api: ApiFn }) {
  ...
  async function run(dry: boolean) {
    if (!dry && !confirm(
      "This rewrites the saved scores of past typing attempts using the fixed " +
      "calculation. Run the check first if you haven't. Continue?"
    )) return;
    setRunning(dry ? "dry" : "real"); setErr(""); setRes(null);
    try {
      setRes(await api(`/tier2/admin/typing/recompute?dry=${dry ? 1 : 0}`, "POST", {}));
    } catch (e: any) {
      setErr(e?.message || "Could not run the repair.");
    } finally { setRunning(""); }
  }

  return (
    <div style={{ ...cardBox, marginTop: 22 }}>
      <h3 style={{ fontSize: 14, fontWeight: 800, margin: "0 0 6px" }}>Repair old typing results</h3>
      <p style={{ fontSize: 12.5, color: MUTED, lineHeight: 1.65, margin: "0 0 12px" }}>
        Scores saved before the scoring fix counted the un-typed part of the passage as
        mistakes, so unfinished attempts show 0% accuracy and 0 net WPM. This re-scores
        them from the text each student actually typed. Safe to run more than once.
      </p>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={() => run(false)} disabled={!!running} style={goldBtn}>
          {running === "real" ? "Repairing…" : "Repair now"}
        </button>
      </div>
            ...
            <div style={{ marginTop: 8, borderTop: `1px solid ${BORDER}`, paddingTop: 8 }}>
              ...
              {res.examples.map((x: any) => (
                <div key={x.attempt_id} style={{ fontFamily: "monospace", fontSize: 11.5 }}>
                  #{x.attempt_id}: {x.before?.accuracy ?? "—"}% / {x.before?.net_wpm ?? "—"} wpm
                  {" → "}
                  <span style={{ color: "#4ade80" }}>
                    {x.after?.accuracy}% / {x.after?.net_wpm} wpm
                  </span>
...
// SKAU Clerk Phase-1 ke shuruaati maanak.
//
// DHYAN: ye niyam RRB NTPC ke hain. SKAU ne apna formula chhaapa hi NAHI —
// students bhi NTPC ke software par hi practice kar rahe hain. Isliye ye
// value patthar ki lakeer nahi, bas ek samajhdaar shuruaat hai. Jis din
// university asli niyam nikaale, passage kholiye aur yahi khaane badal
// dijiye — code ko haath lagane ki zaroorat nahi padegi.
const SKAU_PRESET = {
  duration_min: 10,
  target_wpm: 30,
  min_accuracy: 0,        // NTPC me accuracy ki alag shart hoti hi nahi
  min_keystrokes: 0,      // SKAU ne koi minimum keystroke nahi diya
  passage_mode: "screen",
  ignorable_pct: 5,
...
function PassagesLevel(props: {
  ...
  function checkCsv(text?: string) {
    ...
    body.forEach((r, i) => {
      const [title, num, kind, passage, dur, wpm, acc, lang, mode, keys, pmode] = r;
      ...
      const smode = modeRaw === "keystroke" ? "keystroke" : modeRaw === "ntpc" ? "ntpc" : "word";
      ...
      out.push({
        ...
        // ntpc me accuracy ki shart hai hi nahi — CSV me kuch bhi likha ho,
        // 0 hi jayega. Warna 90 chala jata aur students galat fail hote.
        min_accuracy: ntpc ? 0 : (parseInt((acc || "90").trim(), 10) || 90),
        language: (lang || "english").trim().toLowerCase() === "hindi" ? "hindi" : "english",
        scoring_mode: smode,
        min_keystrokes: ntpc ? 0 : (parseInt((keys || "0").trim(), 10) || 0),
        ...
        // NTPC ke niyam CSV me nahi maangte — column badhane se upload aur
        // uljhta. SKAU wale default apne aap lag jaate hain; alag chahiye to
        // upload ke baad passage kholkar badal lijiye.
        ...(ntpc
          ? { ignorable_pct: 5, mistake_penalty_words: 10,
              marks_max: 15, marks_base_wpm: 30, marks_per_extra_wpm: 0.5 }
          : { ignorable_pct: 0, mistake_penalty_words: 0,
              marks_max: 0, marks_base_wpm: 0, marks_per_extra_wpm: 0 }),
        is_free: false,
        display_order: n,
      ...
      {showBulk ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          ...
          <p style={{ margin: "0 0 10px", fontSize: 12, color: MUTED, lineHeight: 1.6 }}>
            ...
            11va column <b style={{ color: "#e0dacb" }}>Passage mode</b> bhi de sakte hain: paper / screen.
            Blank ho to 10, 30, 90, english, word aur 0 lag jayega, aur keystroke wale apne aap screen par.
            Language: english / hindi. Scoring: word / keystroke / ntpc.
            {" "}<b>ntpc</b> likhne par SKAU wale maanak apne aap lag jaate hain — 5% maaf,
            {" "}10 shabd per galti, 15 marks, 30 WPM base. Alag chahiye to upload ke baad passage kholkar badal lijiye.
        ...
        <div style={{ ...cardBox, marginBottom: 16 }}>
          ...
          {form.scoring_mode !== "ntpc" ? (
            ...
            <div style={{ fontSize: 11.5, color: MUTED, margin: "0 0 12px", lineHeight: 1.6 }}>
              Accuracy ki alag shart nahi lagti — har galti ka hisaab neeche net speed me hi lagta hai.
            </div>
          )}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Field label="Scoring">
              ...
              >
                ...
                <option value="ntpc">Marks out of 15 (NTPC / SKAU)</option>
              </select>
            </Field>
          </div>
          ...
          {form.scoring_mode === "ntpc" && (
            <div style={{ border: `1px solid ${BORDER}`, borderRadius: 10, padding: 12, margin: "0 0 12px" }}>
              <div style={{ fontWeight: 700, fontSize: 13.5, marginBottom: 2 }}>NTPC / SKAU ke niyam</div>
              <div style={{ fontSize: 11.5, color: MUTED, lineHeight: 1.6, marginBottom: 10 }}>
                ...
                <br />
                <b>Ye niyam RRB NTPC ke hain — SKAU ne apna formula chhaapa nahi.</b>{" "}
                Asli notice aaye to yahin badal dijiye.
              </div>
