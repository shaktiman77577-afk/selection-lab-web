...
function PassagesLevel(props: {
  ...
  function blank() {
    return {
      ...
      duration_min: 10, target_wpm: 30, min_accuracy: 90, is_free: false, display_order: 0,
    };
  }
  ...
  function checkCsv(text?: string) {
    ...
    body.forEach((r, i) => {
      const [title, num, kind, passage, dur, wpm, acc] = r;
    ...
    <div>
      ...
      {showBulk ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          ...
          <p style={{ margin: "0 0 10px", fontSize: 12, color: MUTED, lineHeight: 1.6 }}>
            7 columns: <b style={{ color: "#e0dacb" }}>Title, Test number, Type (practice/test), Passage, Minutes, Target WPM, Min accuracy</b>.
            Aakhri teen khaali chhod dein to 10, 30 aur 90 lag jayega.
      ...
      {showForm ? (
        <div style={{ ...cardBox, marginBottom: 16 }}>
          ...
          <Field label="Minimum accuracy (%)" hint="Two conditions to qualify: net WPM >= target AND accuracy >= this. 90% is strict — 80% works better for practice passages.">
