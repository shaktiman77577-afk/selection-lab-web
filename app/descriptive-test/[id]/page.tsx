            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ marginBottom: 10 }}>
                ...
                {sec.questions.length > 1 ? (
                  <div style={{ fontSize: 12.5, color: "var(--muted)", marginTop: 2 }}>Attempt any ONE of the {sec.questions.length} topics — only the selected topic is scored.</div>
                ) : null}
              </div>
              ...
              {sec.questions.length > 1 && (
                <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
                  {sec.questions.map((q, qi) => {
                    ...
                    return (
                      <button key={q.id} onClick={() => setSelected((p) => ({ ...p, [sec.type]: q.id }))}
                        style={{ borderRadius: 10, padding: "9px 14px", fontSize: 13, fontWeight: 800, cursor: "pointer", background: active ? GOLD : "var(--card)", color: active ? "#1a1a1a" : "var(--text)", border: active ? "none" : "1px solid var(--line)" }}>
                        Topic {qi + 1}{done ? " ✓" : ""}
                      </button>
                    );
                  })}
              ...
              <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11, fontWeight: 800, color: "#1a1a1a", background: GOLD, borderRadius: 6, padding: "2px 8px" }}>
                    Question No {allQuestions.findIndex((x) => x.id === selQid) + 1}
                  </span>
                  {selQ?.marks != null ? <span style={{ fontSize: 12, color: "var(--muted)" }}>{selQ.marks} marks</span> : null}
                  {selQid != null && marked[selQid] ? <span style={{ fontSize: 11, fontWeight: 800, color: "#fff", background: "#8e44ad", borderRadius: 6, padding: "2px 8px" }}>Marked for review</span> : null}
                </div>
                ...
                <textarea
                  ...
                  onPaste={(e) => e.preventDefault()}        /* block Ctrl/Cmd+V paste */
                  onDrop={(e) => e.preventDefault()}         /* block drag-and-drop paste */
                  onContextMenu={(e) => e.preventDefault()}  /* kill right-click menu (no Paste option) */
                  onKeyDown={(e) => {
                    // block Tab only (keeps focus in the box).
                    // Enter is allowed so students can make line breaks / paragraphs
                    // (needed for Letters: address, date, salutation, closing).
