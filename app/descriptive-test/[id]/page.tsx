...
export default function DescriptiveTestPlayer() {
  ...
  const [started, setStarted] = useState(false);  // instructions screen → Start test
  const [agree, setAgree] = useState(false);      // "I have read" checkbox
  ...
  useEffect(() => {
    ...
    if (!uid) {
      // must be logged in to attempt any test (even free ones)
      router.replace(`/login?redirect=/descriptive-test/${testId}`);
      return;
    }
    fetch(`${API_URL}/descriptive/test/${testId}${uid ? `?user_id=${uid}` : ""}`)
      .then(async (r) => {
  ...
  useEffect(() => {
    if (!started || phase !== "writing" || loading || locked || !test || paused) return;
    if (secondsLeft <= 0) {
      submit();
      return;
    }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [secondsLeft, phase, loading, locked, test, paused, started]);
  ...
  // ── instructions screen (real exam style) ─────────────────────────────────
  if (!started && phase === "writing") {
    const durMin = Number(test?.duration_min || 40);
    const nameOf = (t: string) => SECTION_LABEL[t] || t;
    const secNames = sections.map((s) => nameOf(s.type)).join(", ");
    const blocks: { head: string; items: string[] }[] = [
      {
        head: "General",
        items: [
          `This test has <b>${sections.length} sections</b>: ${secNames}.`,
          `Total time: <b>${durMin} minutes</b>. The paper is <b>auto-submitted</b> the moment the timer ends.`,
          `Where a section has more than one question, attempt <b>any ONE</b> of them. Tap the question you want to attempt. If you write in both, <b>only the selected question is evaluated</b>.`,
          `Sections with a single question are <b>compulsory</b>.`,
          `The word limit of every question is shown below it. Marks are deducted for writing too short or too long.`,
          `Use the palette button (☰) to move between questions and to mark a question for review.`,
          `Pause stops the timer. Use it only if it is unavoidable.`,
        ],
      },
      {
        head: "Typing rules (same as the actual exam)",
        items: [
          `Press <b>Enter</b> to start a new paragraph. <b>Enter will not work immediately after a space</b> — finish your word first, then press Enter.`,
          `<b>Backspace is the only way to correct your answer.</b> The cursor cannot be moved with the mouse or the arrow keys, and words cannot be selected or double-tapped.`,
          `<b>Copy, cut, paste and drag-drop are disabled.</b>`,
          `<b>Spell-check and auto-correct are switched off</b> — no red underlines and no word suggestions.`,
          `Space, Shift and Caps Lock work normally. The <b>Delete</b> key is disabled.`,
        ],
      },
      {
        head: "Evaluation",
        items: [
          `Every answer is checked on <b>word count, spelling and grammar</b>, and a detailed report with corrections is shown after submission.`,
          `Marks are deducted for grammar and punctuation mistakes.`,
        ],
      },
    ];
    ...
    return (
      <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
        <header style={{ position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: NAVY, color: "#fff" }}>
          ...
          <div style={{ fontWeight: 800, fontSize: 13, background: "rgba(255,255,255,0.14)", borderRadius: 10, padding: "6px 10px" }}>⏱ {durMin} min</div>
        </header>

        <main style={{ maxWidth: 820, margin: "0 auto", padding: "14px 14px 96px" }}>
          <h1 style={{ fontSize: 18, fontWeight: 900, margin: "6px 0 2px" }}>Instructions — please read carefully</h1>
          <p style={{ fontSize: 12.5, color: "var(--muted)", margin: "0 0 14px" }}>
            Candidate: <b style={{ color: "var(--text)" }}>{(getUser() as any)?.name || "Candidate"}</b>
          </p>

          <div style={{ border: "1px solid var(--line)", borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
            <div style={{ display: "flex", background: "var(--header)", fontSize: 11, fontWeight: 800, color: "var(--muted)", textTransform: "uppercase" }}>
              <div style={{ flex: 1, padding: "8px 10px" }}>Section</div>
              <div style={{ width: 84, padding: "8px 10px", textAlign: "center" }}>Given</div>
              <div style={{ width: 84, padding: "8px 10px", textAlign: "center" }}>Attempt</div>
            </div>
            {sections.map((s) => (
              <div key={s.type} style={{ display: "flex", borderTop: "1px solid var(--line)", fontSize: 13.5 }}>
                <div style={{ flex: 1, padding: "9px 10px", fontWeight: 700 }}>{nameOf(s.type)}</div>
                <div style={{ width: 84, padding: "9px 10px", textAlign: "center" }}>{s.questions.length}</div>
                <div style={{ width: 84, padding: "9px 10px", textAlign: "center", fontWeight: 800, color: GOLD }}>
                  {s.questions.length > 1 ? "Any 1" : "1"}
                </div>
              </div>
            ))}
          </div>
          ...
          {blocks.map((blk) => (
            <div key={blk.head} style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 11.5, fontWeight: 900, color: GOLD, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>{blk.head}</div>
              <ol style={{ margin: 0, paddingLeft: 18, fontSize: 13.5, lineHeight: 1.75 }}>
                {blk.items.map((t, i) => (
                  <li key={i} style={{ marginBottom: 4 }} dangerouslySetInnerHTML={{ __html: t }} />
                ))}
              </ol>
            </div>
          ))}

          <div style={{ border: `1px solid ${GOLD}`, background: "rgba(255,171,0,0.08)", borderRadius: 12, padding: "10px 12px", fontSize: 12.5, lineHeight: 1.6 }}>
            <b>Note:</b> These instructions and the paper pattern have been updated as per the <b>new pattern of the Punjab &amp; Haryana High Court examination</b>, based on feedback from students who have appeared in the exam.
          </div>
        </main>

        <div style={{ position: "fixed", left: 0, right: 0, bottom: 0, zIndex: 60, background: "var(--header)", borderTop: "1px solid var(--line)", padding: "10px 14px", display: "flex", alignItems: "center", gap: 10, backdropFilter: "blur(8px)" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, flex: 1, cursor: "pointer" }}>
            <input type="checkbox" checked={agree} onChange={(e) => setAgree(e.target.checked)} style={{ width: 18, height: 18, accentColor: GOLD, flexShrink: 0 }} />
            I have read and understood all the instructions.
          </label>
          <button onClick={() => setStarted(true)} disabled={!agree}
            style={{ ...goldBtn, whiteSpace: "nowrap", opacity: agree ? 1 : 0.5, cursor: agree ? "pointer" : "not-allowed" }}>
            Start test ▸
          </button>
        </div>
      </div>
                    ...
                    {(!multi || active) ? (
                      <>
                        <textarea
                          ...
                          spellCheck={false}                          /* no spelling hints — real exam */
                          autoCorrect="off"
                          autoCapitalize="off"
                          autoComplete="off"
                          ...
                          onMouseDown={(e) => {
                            // Click se cursor beech me set nahi hoga.
                          ...
                          onSelect={(e) => {
                            // Double/triple click ya drag se selection bane to
                            // turant collapse karke caret end par bhej do.
                            const el = e.currentTarget;
                          ...
                          onBeforeInput={(e) => {
                            // Mobile keyboard par keydown reliable nahi hota,
                            // isliye Enter ka rule yahan bhi lagaya hai.
                            const el = e.currentTarget as HTMLTextAreaElement;
                            const ne = e.nativeEvent as any;
                            const it: string = (ne && ne.inputType) || "";
                            const data: string = (ne && ne.data) || "";
                            const isBreak = it === "insertLineBreak" || data.includes("\n");
                            if (isBreak && (el.value.length === 0 || /[ \t\n]$/.test(el.value))) {
                              e.preventDefault();
                            }
                            if (it.startsWith("insertFromPaste")) e.preventDefault();
                          }}
                          onKeyDown={(e) => {
                            const el = e.currentTarget;
                            const k = e.key;

                            // 1) Cursor move karne wali saari keys band —
                            // student sirf end par hi likh sakta hai.
                            if (["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown", "Home", "End", "PageUp", "PageDown", "Tab", "Delete"].includes(k)) {
                              e.preventDefault();
                              return;
                            }

                            // 2) Ctrl/Cmd shortcuts band (select all, copy, cut, paste, undo, redo)
                            const lk = k.toLowerCase();
                            if ((e.ctrlKey || e.metaKey) && ["a", "c", "x", "v", "z", "y", "insert"].includes(lk)) {
                              e.preventDefault();
                              return;
                            }

                            // 3) Space ke turant baad Enter band —
                            // real exam me bhi space dabane ke baad Enter nahi chalta.
                            if (k === "Enter") {
                            ...
                            // 4) Kisi tarah selection ban gayi ho to pehle caret
                            // end par le jao, tabhi key apply ho.
                            const n = el.value.length;
                            if (el.selectionStart !== n || el.selectionEnd !== n) el.setSelectionRange(n, n);

                            // Enter (sahi jagah), Space, Backspace, Caps Lock — normal chalte hain.
                          }}
                        ...
                        <div style={{ marginTop: 4, fontSize: 11, color: "var(--muted)" }}>
                          ✍️ Type your answer — press Enter for a new paragraph (Enter will not work right after a space). Editing is backspace-only. Paste is disabled for a fair test.
                        </div>
                      </>
                    ) : hasDraft ? (
      ...
      {results.map((r: any, i: number) => {
        ...
        return (
          <div key={r.question_id} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, padding: 16, boxShadow: "var(--shadow)", marginTop: 16 }}>
            ...
            <ScoreBar label="Spelling" got={s.spelling_score} max={s.max_spelling_marks} extra={`${s.spelling_correct ?? 0}/${s.spelling_total ?? 0} correct${s.length_capped ? " · scaled to answer length" : ""}`} />
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <div style={{ background: "var(--bg)", border: "1px solid var(--line)", borderRadius: 10, padding: 12, fontSize: 13.5, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                {r.your_answer ? (
                  <AnnotatedAnswer text={r.your_answer} grammar={s.grammar_issues || []} spelling={s.spelling_issues || []} />
                ) : (
                  <span style={{ color: "var(--muted)" }}>(left blank)</span>
                )}
              </div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>
                <span style={{ borderBottom: "2px solid #2980b9" }}>blue underline</span> = grammar/punctuation &nbsp;·&nbsp; <span style={{ borderBottom: "2px solid #c0392b" }}>red underline</span> = spelling
              </div>
            </div>
...
function AnnotatedAnswer({ text, grammar, spelling }: { text: string; grammar: any[]; spelling: any[] }) {
  type Mark = { start: number; end: number; color: string; title: string };
  const marks: Mark[] = [];
  const push = (it: any, color: string) => {
    const start = Number(it.offset);
    if (!Number.isFinite(start) || start < 0 || start >= text.length) return;
    const end = Math.min(text.length, start + Math.max(1, Number(it.length) || 1));
    marks.push({ start, end, color, title: it.message || it.short || "" });
  };
  (grammar || []).forEach((it) => push(it, "#2980b9"));
  (spelling || []).forEach((it) => push(it, "#c0392b"));
  marks.sort((a, b) => a.start - b.start);
  // drop overlaps
  const clean: Mark[] = [];
  let last = -1;
  for (const m of marks) {
    if (m.start >= last) { clean.push(m); last = m.end; }
  }
  ...
  clean.forEach((m, i) => {
    ...
    out.push(
      <span key={`m${i}`} title={m.title} style={{ borderBottom: `2px solid ${m.color}`, textDecorationSkipInk: "none", background: `${m.color}14` }}>
  ...
  if (s?.grammar_checked === false) {
    ...
    body = (
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {typeof s.grammar_deduction === "number" && s.grammar_deduction > 0 ? (
          <div style={{ ...box, color: "#c0392b", fontWeight: 700 }}>
            Total deducted for grammar/punctuation mistakes: −{s.grammar_deduction} marks ({issues.length} issue{issues.length === 1 ? "" : "s"} × 0.1)
          </div>
        ) : null}
        {issues.map((it, i) => {
          const sugg = (it.suggestions || []).filter(Boolean);
          ...
          return (
            <div key={i} style={box}>
              <div style={{ fontWeight: 700 }}>
                ...
                {typeof it.deduction === "number" ? (
                  <span style={{ float: "right", color: "#c0392b", fontWeight: 800, fontSize: 12 }}>−{it.deduction} mark{it.deduction === 1 ? "" : "s"}</span>
                ) : null}
              </div>
