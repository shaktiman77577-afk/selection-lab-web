    ...
    return (
      <Shell>
        <div style={{ maxWidth: 560, margin: "0 auto", paddingTop: 12 }}>
          ...
          <Rule n="1" head="Speed kaise nikalti hai">
            Aapke kul keystrokes ko 5 se bhaag dete hain — yahi <b>gross words</b> hain.
            Phir galtiyan ghatti hain, aur jo bachta hai use test ke minute se bhaag
            dete hain. Wahi <b>net WPM</b> hai.
          </Rule>

          <Rule n="2" head="Galti poori ya aadhi">
            Spelling, chhoota hua shabd, extra shabd aur punctuation — <b>poori galti</b>.
            <br />
            Spacing aur capital letter — <b>aadhi galti</b>.
          </Rule>

          <Rule n="3" head={`${meta.ignorable_pct}% galtiyan maaf hain`}>
            Gross words ka {meta.ignorable_pct}% tak galtiyan bilkul nahi ginti.
            600 shabd par lagbhag {Math.floor(600 * (Number(meta.ignorable_pct) || 0) / 100)} galtiyan maaf.
          </Rule>

          <Rule n="4" head={`Chhoot ke baad har galti ${meta.mistake_penalty_words} shabd kha jaati hai`} warn>
            Yahi wo jagah hai jahan score toot-ta hai. Ek example —
            {(() => {
              ...
              return (
                <div style={{ marginTop: 8, fontSize: 12.5, lineHeight: 1.85, color: "var(--muted)" }}>
                  {gross} gross words par {free} galtiyan maaf hain.
                  <br />
                  <b>{free} galti:</b> chhoot ke andar → poore {gross} shabd bache → <b>{Math.round(gross / mins)} WPM</b>
                  <br />
                  <b>{over} galti:</b> 10 extra × {pen} = {lost} shabd gaye → {gross - lost} → <b>{Math.round((gross - lost) / mins)} WPM</b>
                </div>
              );
            })()}
            <div style={{ marginTop: 8 }}>
              Sirf 10 galti zyada, aur speed {Math.round((10 * (Number(meta.mistake_penalty_words) || 0)) / (Number(meta.duration_min) || 10))} WPM gir gayi.
              Isliye teji se zyada zaroori hai ki haath na phisle.
            </div>
          </Rule>

          <Rule n="5" head="Marks">
            {base} WPM par 0 marks, uske baad har extra WPM ke {per} marks.
            ...
            <div style={{ marginTop: 10 }}>
              <b>{fullAt} WPM par poore {max} marks mil jaate hain.</b> Usse tez type karne
              ka ek bhi extra number nahi milta — us mehnat ko accuracy par lagaiye.
            </div>
          </Rule>
          ...
          <div style={{
            ...
            University ne typing test ka apna formula prakashit nahi kiya hai. Upar diya
            gaya hisaab RRB NTPC ke pattern par hai, jo is samay in exams me sabse zyada
            chalta hai. Marks ka niyam ({base} WPM ke baad har WPM par {per}, adhiktam {max})
            official notification se liya gaya hai.
          </div>
          ...
          <div style={{ fontSize: 11.5, color: "var(--muted)", textAlign: "center", marginTop: 8 }}>
            Timer pehla akshar type karte hi chalega, is button par nahi.
          </div>
        </div>
      </Shell>
    );
    ...
    <Shell>
      <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{ border: "1px solid var(--border)", borderRadius: 14, padding: 14, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, fontSize: 14, paddingTop: 9 }}>
              <span style={{ fontWeight: 700 }}>
                Marks ({r.net_wpm} → {Math.floor(Number(r.net_wpm) || 0)} − {r.marks_base_wpm}) × {r.marks_per_extra_wpm}
              </span>
              <b style={{ flexShrink: 0, color: "#2e8b4a" }}>{r.marks} / {r.marks_max}</b>
            </div>
            ...
            <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 10, lineHeight: 1.7 }}>
              {Number(r.net_mistakes) <= 0
                ? `Aapki ${r.total_mistakes} galtiyan ${r.ignorable_mistakes} ki chhoot ke andar thin — ek bhi shabd nahi kata.`
                : `Chhoot ${r.ignorable_mistakes} galti ki thi, aapki ${r.total_mistakes} hui. ${r.net_mistakes} extra × ${r.mistake_penalty_words} = ${Math.round(Number(r.net_mistakes) * Number(r.mistake_penalty_words))} shabd kat gaye.`}
            </div>
          </div>
        )}
        ...
        <p style={{ fontSize: 12, color: "var(--muted)", marginTop: -6, marginBottom: 16 }}>
          ...
          <span style={{ color: GREY, fontWeight: 700 }}>Grey</span> = not reached.<br />Kisi bhi laal ya neele shabd par tap kijiye — dikhega ki aapne kya likha tha.
        </p>
...
const WHY_TEXT: Record<string, string> = {
  spelling: "spelling ki galti",
  punctuation: "sirf punctuation ka farak — comma, full stop wagairah",
  caps: "sirf chhote-bade akshar ka farak",
  format: "spacing ki galti — ek se zyada space pad gaya",
  missing: "ye shabd type hi nahi hua",
  extra: "ye shabd passage me hai hi nahi",
};
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {s.op === "missing" ? (
            <div style={{ fontSize: 13.5, lineHeight: 1.7 }}>
              ...
              {" "}— ye shabd aapne type hi nahi kiya
            </div>
          ) : s.op === "extra" ? (
            <div style={{ fontSize: 13.5, lineHeight: 1.7 }}>
              Aapne <b style={{ fontFamily: "Consolas, monospace", color: RED }}>{s.typed}</b>
              {" "}likha, par ye passage me hai hi nahi
            </div>
          ) : (
            <div style={{ fontSize: 13.5, lineHeight: 1.7 }}>
              <b style={{ fontFamily: "Consolas, monospace", color: GREEN }}>{s.text}</b>
              {" "}ki jagah aapne{" "}
              <b style={{ fontFamily: "Consolas, monospace", color: RED }}>{s.typed || "(kuch nahi)"}</b>
              {" "}likha
            </div>
          )}
          {why && (
            <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 5 }}>
              {why}{half ? " — aadhi galti" : ""}
            </div>
          )}
        </div>
        <button
...
function HowCalculated({ r }: { r: any }) {
  ...
  const rows: [string, string][] = ntpcMode
    ? [
        ...
        ["Marks", `(${Math.floor(Number(r.net_wpm) || 0)} − ${r.marks_base_wpm}) × ${r.marks_per_extra_wpm} = ${r.marks} / ${r.marks_max}`],
        ["Qualifying speed", `${r.target_wpm} WPM — accuracy ki alag shart nahi`],
