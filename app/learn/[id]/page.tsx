...
export default function LearnPage() {
  ...
  // PDF blob URL — API ka asli link browser me kabhi expose nahi hota
  ...
  // Active PDF ko blob me laate hain: iframe ko blob: URL milta hai, isliye
  // address bar aur "open in new tab" me api.selectionlab.online ka link nahi
  // jaata. Har badlav pe purana blob revoke ho jata hai.
  useEffect(() => {
    let created = "";
    setPdfErr("");
    setPdfBlob("");
    const isPdf = active && String(active.content_type).toLowerCase() === "pdf";
    if (!isPdf || !user?.id) return;
    setPdfLoading(true);
    fetch(`${API_URL}/secure-pdf/${active!.id}?user_id=${user.id}`)
      .then(async (r) => {
        if (!r.ok) {
          let msg = `Could not load PDF (${r.status})`;
          try {
      ...
      .catch((e: any) => setPdfErr(e?.message || "Could not load PDF"))
            ...
            {active && (
              <div style={{ marginBottom: 16 }}>
                {active.content_type === "video" ? (
                  ...
                  // PDF: blob se dikhate hain — API ka link na address bar me
                  // jaata hai, na "open in new tab" me. Watermark backend lagata hai.
                  <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden" }}>
                    ...
                    {!!pdfErr && !pdfLoading && (
                      <div style={{ padding: 30, textAlign: "center" }}>
                        ...
                        <p style={{ fontSize: 14, color: "#e05555" }}>{pdfErr}</p>
                      </div>
                    )}
                    {!!pdfBlob && !pdfLoading && (
                      <>
                        <iframe
                          src={pdfBlob}
                          title={active.title}
                          style={{ width: "100%", height: "70vh", minHeight: 420, border: "none", background: "#fff" }}
                        />
                        <div style={{ padding: "12px 14px", borderTop: "1px solid var(--line)", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                          <a
                            href={pdfBlob}
                            download={`${(active.title || "notes").replace(/[^\w\s-]/g, "").trim()}.pdf`}
                            style={{ background: GOLD, color: "#1a1a1a", borderRadius: 10, padding: "10px 18px", fontWeight: 800, fontSize: 13.5, textDecoration: "none" }}
                          >
                            ⬇ Download PDF
                          </a>
                          <span style={{ fontSize: 11.5, color: "var(--muted)", flex: 1, minWidth: 200 }}>
