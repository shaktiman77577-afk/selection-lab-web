            ...
            {active && (
              <div style={{ marginBottom: 16 }}>
                {active.content_type === "video" ? (
                  ...
                  // PDF: raw Supabase link kabhi nahi. Backend har user ko uske
                  // apne mobile number ka watermark laga ke PDF deta hai.
                  <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden" }}>
                    <iframe
                      src={`${API_URL}/secure-pdf/${active.id}?user_id=${user?.id ?? ""}`}
                      title={active.title}
                    ...
                    <div style={{ padding: "12px 14px", borderTop: "1px solid var(--line)", display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                      <a
                        href={`${API_URL}/secure-pdf/${active.id}?user_id=${user?.id ?? ""}`}
                        target="_blank"
                        rel="noreferrer"
                        style={{ background: "var(--card)", color: "var(--text)", border: "1px solid var(--line)", borderRadius: 10, padding: "10px 16px", fontWeight: 700, fontSize: 13.5, textDecoration: "none" }}
                      >
                        Open in new tab ↗
                      </a>
                      <a
                        href={`${API_URL}/secure-pdf/${active.id}?user_id=${user?.id ?? ""}&download=1`}
