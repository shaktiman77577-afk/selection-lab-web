...
export default function Tier2Page() {
  ...
  async function openSeries(s: Series) {
    ...
    // Seedha checkout kholne se free trial tak pahunchne ka rasta hi band ho
    // jata tha.
    setOpen(s);
    setDetail(null);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      ...
      >
        <button
          onClick={() => (open ? setOpen(null) : setMenuOpen(true))}
          aria-label={open ? "Back" : "Menu"}
          style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: "var(--text)", padding: 4 }}
        >
          {open ? "←" : "☰"}
        </button>
        <div style={{ fontWeight: 800, fontSize: 17, flex: 1 }}>{open ? open.title : "Tier 2 Practice"}</div>
        <ThemeToggle />
      </header>
            ...
            {!detail ? (
              ...
              <>
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 3, lineHeight: 1.5 }}>
                        Free wale test pehle dekh lijiye. Baaki sab kharidne par khulega.
                      </div>
                    </div>
                    <button
                      onClick={() => { if (!getUser()) { router.push("/login"); return; } setBuying(open); }}
                ...
                <div
                  ...
                  style={{
                    background: "var(--card)", border: "1px solid var(--line)", borderRadius: 14,
                    padding: 14, marginTop: 14, marginBottom: 4, cursor: "pointer",
                    display: "flex", alignItems: "center", gap: 12,
                  }}
                >
                  <div style={{ fontSize: 26 }}>📊</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 14.5, fontWeight: 800 }}>Excel / CPT practice</div>
                    <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 3, lineHeight: 1.5 }}>
                      38 formula chart, guided practice and 10 mock tests
                    </div>
                  </div>
