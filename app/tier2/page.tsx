                  ...
                  return (
                    <>
                      ...
                      {practiceList.map((p: Passage) => (
                        <PassageRow key={p.id} p={p} uid={uid} router={router} scriptQS={scriptQS} onLocked={openBuy} />
                      ))}
                      ...
                      {testsList.map((p: Passage) => (
                        <PassageRow key={p.id} p={p} uid={uid} router={router} scriptQS={scriptQS} isTest onLocked={openBuy} />
                      ))}
                      {testsList.length === 0 && (
                        <p style={{ color: "var(--muted)", fontSize: 14 }}>
...
// ── Ek passage ki row ───────────────────────────────────────────────────────
function PassageRow({ p, uid, router, isTest, scriptQS, onLocked }: { p: Passage; uid?: number; router: any; isTest?: boolean; scriptQS?: string; onLocked: () => void }) {
  return (
    <div
      style={{
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
        ...
        {p.unlocked ? (
          <button
            onClick={() => router.push(`/typing-test/${p.id}${scriptQS || ""}`)}
