...
export default function ExcelHub() {
  ...
  const [tab, setTab] = useState<"chart" | "practice" | "tests" | "worksheet">("chart");
  // Chautha option — chhape hue page se poori sheet banana. Baaki teen
  // apni jagah waise ke waise hain; ye unka data bhi nahi chhoota, apna
  // alag endpoint use karta hai.
  ...
  // Kaunsi series? Link me ?s=<id> ho (NBEMS ke "Computer skills" se) to
  // wahi — warna pehle hamesha pehli series (P&H) khulti thi aur NBEMS me
  // court wale worksheets dikhte the. Hidden series list me aati hi nahi,
  // isliye ID seedha lete hain aur naam series ke apne endpoint se.
  useEffect(() => {
    const want = Number(new URLSearchParams(window.location.search).get("s") || 0);
    if (want > 0) {
      setSeriesId(want);
      fetch(`${API_URL}/tier2/series/${want}${uid ? `?user_id=${uid}` : ""}`)
        .then((r) => r.json())
  ...
  const load = useCallback(() => {
    ...
    // Worksheet ka fetch JAAN-BOOJH KAR upar wale Promise.all se bahar hai.
    // Usme daal dete to ye endpoint fail hone par (jaise backend deploy hone
    // se pehle) poora Promise.all toot jata aur TEENO purane option khaali
    // dikhne lagte. Alag rakhne se worksheet na chale to bhi baaki sab
    // pehle jaisa chalta rahega.
    fetch(`${API_URL}/tier2/excel/worksheets?series_id=${seriesId}${u}`)
      .then((r) => r.json())
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      ...
      <div style={{ display: "flex", gap: 6, padding: "10px 14px 0", maxWidth: 900, margin: "0 auto" }}>
        {(["chart", "practice", "tests", "worksheet"] as const).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
          ...
          >
            {t === "chart" ? "Formula chart" : t === "practice" ? "Practice" : t === "tests" ? "Tests" : "Worksheet"}
          </button>
        ))}
      </div>
      ...
      <main style={{ maxWidth: 900, margin: "0 auto", padding: "14px 14px 40px" }}>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {p.instruction_en || p.instruction_hi}
                    </div>
                  </div>
                ...
                {t.unlocked ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
                    <button
                      ...
                      style={{ background: GOLD, color: "#1a1a1a", border: "none", borderRadius: 9, padding: "9px 16px", fontWeight: 800, fontSize: 12.5, cursor: "pointer" }}
                    >
                      Start
                    </button>
                    {/* Question paper — print karke bhi practice ho sake.
                        Answer key nahi; student ke number ka watermark. */}
                    {uid ? (
                      <a
                        href={`${API_URL}/tier2/excel/test/pdf/${t.id}?user_id=${uid}`}
                        style={{ border: `1px solid ${GOLD}`, color: GOLD, borderRadius: 9, padding: "6px 10px", fontWeight: 800, fontSize: 11.5, textAlign: "center", textDecoration: "none" }}
                      >
                        📄 PDF
                      </a>
                    ) : null}
        ...
        {!loading && tab === "worksheet" && (
          <>
            <div style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 13, marginBottom: 12, fontSize: 12.5, lineHeight: 1.75, color: "var(--muted)" }}>
              Here you <b style={{ color: "var(--text)" }}>build the sheet yourself</b>.
              The question paper comes as a PDF: print it, keep it in front of you,
              and type the whole sheet. The sheet is not shown on screen, just like
              in the real exam.
            </div>
            {worksheets.map((t) => (
              <div key={t.id} style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 13, marginBottom: 10, display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: "var(--muted)", marginTop: 2 }}>
                    {t.questions} instructions · {t.duration_min} min · {t.start_cell} se shuru · pass {t.pass_marks}
                    {t.best ? ` · best ${t.best.score}/${t.best.total}` : ""}
                  </div>
                </div>
                {t.unlocked ? (
                  <button
                    onClick={() => router.push(`/worksheet-test/${t.id}`)}
            ...
            {worksheets.length === 0 && <p style={{ color: "var(--muted)", fontSize: 14 }}>Worksheet tests coming soon.</p>}
          </>
        )}
      </main>
    </div>
