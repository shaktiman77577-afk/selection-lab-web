  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      <header style={{ position: "sticky", top: 0, zIndex: 50, display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "var(--header)", backdropFilter: "blur(8px)", borderBottom: "1px solid var(--line)" }}>
        <button onClick={() => router.push("/tier2")} aria-label="Back" style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: "var(--text)", padding: 4 }}>←</button>
        ...
        {!loading && tab === "practice" && (
          <>
            ...
            {practice.map((p) => (
              <div
                key={p.id}
                onClick={() => router.push(`/excel-practice/${p.id}`)}
                style={{ background: "var(--card)", border: "1px solid var(--line)", borderRadius: 12, padding: 12, marginBottom: 10, display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}
              >
                <span style={{ fontSize: 16 }}>{p.done ? "✅" : "⬜"}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: "Consolas, monospace", fontWeight: 800, fontSize: 13.5, color: GOLD }}>{p.concept}</div>
