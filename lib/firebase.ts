...
const firebaseConfig = {
  ...
  // authDomain = our OWN primary domain (Vercel redirects everything to www),
  // proxied to firebaseapp.com via next.config.ts rewrites. First-party handshake
  // → works on mobile browsers that block third-party storage. MUST match the
  // Vercel primary domain exactly, else Google returns redirect_uri_mismatch.
  authDomain: "www.selectionlab.in",
...
// Flow: sendOtp(phone) -> user ko SMS -> verifyOtp(code) -> Firebase ID token
// -> backend /users/login-phone token verify karke login karata hai.

let confirmation: ConfirmationResult | null = null;
let recaptcha: RecaptchaVerifier | null = null;

/** Invisible reCAPTCHA — Firebase Phone Auth ke liye zaroori hai (spam rokta hai). */
...
/** reCAPTCHA reset — OTP fail hone ke baad dobara try karne se pehle zaroori. */
...
/**
 * phone: 10-digit Indian number (jaise "9876543210"). +91 hum khud lagate hain.
 * Success -> SMS chala gaya, ab verifyOtp() call karo.
 */
export async function sendOtp(phone: string): Promise<void> {
  const digits = (phone || "").replace(/\D/g, "").slice(-10);
  if (digits.length !== 10) throw new Error("Please enter a valid 10-digit mobile number");
  confirmation = await signInWithPhoneNumber(auth, "+91" + digits, getRecaptcha());
}

/**
 * code: user ne jo 6-digit OTP daala.
 * Returns: Firebase ID token — ise backend ko bhejna hai.
 */
export async function verifyOtp(code: string): Promise<string> {
  if (!confirmation) throw new Error("OTP session expired. Please request a new code.");
  const cred = await confirmation.confirm((code || "").trim());
  return await cred.user.getIdToken();
}

/** Google se logged-in user ka phone verify karne ke liye (link flow). */
